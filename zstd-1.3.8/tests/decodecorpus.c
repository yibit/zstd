...
/* Write `size` bytes into `ptr`, all of which are less than or equal to
 * `maxSymb` */
static void
RAND_bufferMaxSymb(U32 *seed, void *ptr, size_t size, int maxSymb)
{
    size_t i;
    BYTE *op = ptr;
...
static void
RAND_buffer(U32 *seed, void *ptr, size_t size)
{
    size_t i;
    BYTE *op = ptr;
...
static void
RAND_bufferDist(U32 *seed, BYTE *dist, void *ptr, size_t size)
{
    size_t i;
    BYTE *op = ptr;
...
static void
RAND_genDist(U32 *seed, BYTE *dist, double weight)
{
    size_t i = 0;
    ...
    BYTE step =
        (BYTE)((RAND(seed) % 256) |
               1); /* force it to be odd so it's relatively prime to 256 */
...
#define MAX_WINDOW_LOG \
    22 /* Recommended support is 8MB, so limit to 4MB + mantissa */
...
BYTE SEQUENCE_LITERAL_BUFFER
    [ZSTD_BLOCKSIZE_MAX]; /* storeSeq expects a place to copy literals to */
...
typedef struct {
    size_t contentSize;  /* 0 means unknown (unless contentSize == windowSize ==
                            0) */
...
static void
writeFrameHeader(U32 *seed, frame_t *frame, dictInfo info)
{
...

    {
        ...
        if (RAND(seed) & 7 && g_maxDecompressedSizeLog > 7) {
            ...
            /* do small content */
            highBit = 1ULL << RAND_range(
                          seed, 0, MIN(7, 1U << g_maxDecompressedSizeLog));
        } else {
            /* 0 size frame */
            highBit = 0;
        }
    ...
    if (contentSizeFlag) {
        ...
        if (fcsCode == 1 && fh.contentSize < 256)
            fcsCode++;
    }
    ...
    {
        /*
         * fcsCode: 2-bit flag specifying how many bytes used to represent
         * Frame_Content_Size (bits 7-6)
         * singleSegment: 1-bit flag describing if data must be regenerated
         * within a single continuous memory segment. (bit 5)
         * contentChecksumFlag: 1-bit flag that is set if frame includes
         * checksum at the end -- set to 1 below (bit 2)
         * dictBits: 2-bit flag describing how many bytes Dictionary_ID uses --
         * set to 3 (bits 1-0)
    ...
    if (contentSizeFlag) {
        ...
        default: /* Impossible */
        case 0:
            op[pos++] = (BYTE)fh.contentSize;
            break;
        case 1:
            MEM_writeLE16(op + pos, (U16)(fh.contentSize - 256));
            pos += 2;
            break;
        case 2:
            MEM_writeLE32(op + pos, (U32)fh.contentSize);
            pos += 4;
            break;
        case 3:
            MEM_writeLE64(op + pos, (U64)fh.contentSize);
            pos += 8;
            break;
        }
    }
...
static size_t
writeLiteralsBlockSimple(U32 *seed, frame_t *frame, size_t contentSize)
{
    ...
    if (frame->src == frame->srcStart && litSize == 0) {
        litSize =
            1; /* no empty literals if there's nothing preceding this block */
    }
...
static size_t
writeHufHeader(U32 *seed, HUF_CElt *hufTable, void *dst, size_t dstSize,
               const void *src, size_t srcSize)
{
    ...
    /* Scan input and build symbol stats */
    {
        size_t const largest =
            HIST_count_wksp(count, &maxSymbolValue, (const BYTE *)src, srcSize,
                            WKSP, sizeof(WKSP));
    ...
    {
        size_t const maxBits = HUF_buildCTable_wksp(
            hufTable, count, maxSymbolValue, huffLog, WKSP, sizeof(WKSP));
        CHECKERR(maxBits);
        huffLog = (U32)maxBits;
    }
    ...
    {
        size_t const hSize =
            HUF_writeCTable(op, dstSize, hufTable, maxSymbolValue, huffLog);
        if (hSize + 12 >= srcSize)
            return 0; /* not useful to try compression */
        op += hSize;
    }
...
static size_t
writeLiteralsBlockCompressed(U32 *seed, frame_t *frame, size_t contentSize)
{
    ...
    do {
        ...
        if ((RAND(seed) & 3) || !frame->stats.hufInit) {
            do {
                ...
                hufHeaderSize =
                    writeHufHeader(seed, (HUF_CElt *)frame->stats.hufTable, op,
                                   opend - op, frame->stats.hufDist, DISTSIZE);
...
static size_t
writeLiteralsBlock(U32 *seed, frame_t *frame, size_t contentSize)
{
...
static inline void
initSeqStore(seqStore_t *seqStore)
{
                  ...
                  size_t contentSize, size_t literalsSize, dictInfo info)
{
    ...
    for (i = 0; i < numSequences; i++) {
        ...
        U32 literalLen =
            (RAND(seed) & 7)
                ? ROUND(
                      RAND_exp(seed, literalsSize / (double)(numSequences - i)))
                : 0;
        ...
        if (i == 0 && srcPtr == frame->srcStart && literalLen == 0)
            literalLen = 1;
        if (i + 1 == numSequences)
            matchLen = MIN_SEQ_LEN + (U32)excessMatch;
        ...
        do {
            if (RAND(seed) & 7) {
                /* do a normal offset */
                U32 const dataDecompressed =
                    (U32)((BYTE *)srcPtr - (BYTE *)frame->srcStart);
                offset =
                    (RAND(seed) %
                ...
                if (info.useDict && (RAND(seed) & 1) && i + 1 != numSequences &&
                    dataDecompressed < frame->header.windowSize) {
                    /* need to occasionally generate offsets that go past the
                     * start */
                    /* including i+1 != numSequences because the last sequences
                     * has to adhere to predetermined contentSize */
                    U32 lenPastStart = (RAND(seed) % info.dictContentSize) + 1;
                    offset = (U32)((BYTE *)srcPtr - (BYTE *)frame->srcStart) +
                             lenPastStart;
                    if (offset > frame->header.windowSize) {
                        if (lenPastStart < MIN_SEQ_LEN) {
                            /* when offset > windowSize, matchLen bound by end
                             * of dictionary (lenPastStart) */
                            /* this also means that lenPastStart must be greater
                             * than MIN_SEQ_LEN */
                            /* make sure lenPastStart does not go past
                             * dictionary start though */
                            lenPastStart = MIN(lenPastStart + MIN_SEQ_LEN,
                                               (U32)info.dictContentSize);
                            offset = (U32)((BYTE *)srcPtr -
                                           (BYTE *)frame->srcStart) +
                                     lenPastStart;
                        }
                        {
                            U32 const matchLenBound =
                                MIN(frame->header.windowSize, lenPastStart);
            ...
            ((!info.useDict) &&
             (offset > (size_t)((BYTE *)srcPtr - (BYTE *)frame->srcStart))) ||
            offset == 0);

        {
            ...
            for (j = 0; j < matchLen; j++) {
                if ((U32)((BYTE *)srcPtr - (BYTE *)frame->srcStart) < offset) {
                    ...
                    size_t const dictOffset =
                        offset - (srcPtr - (BYTE *)frame->srcStart);
        ...
        DISPLAYLEVEL(6, "      LL: %5u OF: %5u ML: %5u", (unsigned)literalLen,
                     (unsigned)offset, (unsigned)matchLen);
        DISPLAYLEVEL(7, " srcPos: %8u seqNb: %3u",
                     (unsigned)((BYTE *)srcPtr - (BYTE *)frame->srcStart),
                     (unsigned)i);
        DISPLAYLEVEL(6, "\n");
        if (offsetCode < 3) {
...
static void
initSymbolSet(const BYTE *symbols, size_t len, BYTE *set, BYTE maxSymbolValue)
{
    size_t i;
...
static int
isSymbolSubset(const BYTE *symbols, size_t len, const BYTE *set,
               BYTE maxSymbolValue)
{
    size_t i;
...
static size_t
writeSequences(U32 *seed, frame_t *frame, seqStore_t *seqStorePtr, size_t nbSeq)
{
    ...
    /* CTable for Literal Lengths */
    {
        ...
        size_t const mostFrequent =
            HIST_countFast_wksp(count, &max, llCodeTable, nbSeq, WKSP,
                                sizeof(WKSP)); /* cannot fail */
        ...
        if (mostFrequent == nbSeq) {
            ...
            *op++ = llCodeTable[0];
            ...
            FSE_buildCTable_wksp(CTable_LitLength, LL_defaultNorm, MaxLL,
                                 LL_defaultNormLog, scratchBuffer,
            ...
            FSE_normalizeCount(norm, tableLog, count, nbSeq_1, max);
            {
                size_t const NCountSize =
                    FSE_writeNCount(op, oend - op, norm, max,
                                    tableLog); /* overflow protected */
                if (FSE_isError(NCountSize))
                    return ERROR(GENERIC);
                op += NCountSize;
            }
            FSE_buildCTable_wksp(CTable_LitLength, norm, max, tableLog,
    ...
    /* see Literal Lengths for descriptions of mode choices */
    {
        unsigned max = MaxOff;
        size_t const mostFrequent =
            HIST_countFast_wksp(count, &max, ofCodeTable, nbSeq, WKSP,
                                sizeof(WKSP)); /* cannot fail */
        assert(!HIST_isError(mostFrequent));
        if (mostFrequent == nbSeq) {
            *op++ = ofCodeTable[0];
            ...
            FSE_buildCTable_wksp(CTable_OffsetBits, OF_defaultNorm,
                                 DefaultMaxOff, OF_defaultNormLog,
                                 scratchBuffer, sizeof(scratchBuffer));
            Offtype = set_basic;
        } else {
            size_t nbSeq_1 = nbSeq;
            ...
            FSE_buildCTable_wksp(CTable_OffsetBits, norm, max, tableLog,
                                 scratchBuffer, sizeof(scratchBuffer));
    ...
    /* see Literal Lengths for descriptions of mode choices */
    {
        unsigned max = MaxML;
        size_t const mostFrequent =
            HIST_countFast_wksp(count, &max, mlCodeTable, nbSeq, WKSP,
                                sizeof(WKSP)); /* cannot fail */
        assert(!HIST_isError(mostFrequent));
        if (mostFrequent == nbSeq) {
            *op++ = *mlCodeTable;
            ...
            FSE_buildCTable_wksp(CTable_MatchLength, ML_defaultNorm, MaxML,
                                 ML_defaultNormLog, scratchBuffer,
                                 sizeof(scratchBuffer));
            MLtype = set_basic;
        } else {
            /* fall back on table */
            ...
            FSE_buildCTable_wksp(CTable_MatchLength, norm, max, tableLog,
                                 scratchBuffer, sizeof(scratchBuffer));
...
static size_t
writeSequencesBlock(U32 *seed, frame_t *frame, size_t contentSize,
                    size_t literalsSize, dictInfo info)
{
    ...
    numSequences = generateSequences(seed, frame, &seqStore, contentSize,
                                     literalsSize, info);
...
static size_t
writeCompressedBlock(U32 *seed, frame_t *frame, size_t contentSize,
                     dictInfo info)
{
...
static void
writeBlock(U32 *seed, frame_t *frame, size_t contentSize, int lastBlock,
           dictInfo info)
{
    ...
    if (blockTypeDesc == 0) {
        ...
        if (compressedSize >= contentSize) { /* compressed block must be
    ...
    header[0] =
        (BYTE)((lastBlock | (blockType << 1) | (blockSize << 3)) & 0xff);
...
static void
writeBlocks(U32 *seed, frame_t *frame, dictInfo info)
{
    ...
    while (1) {
        ...
        int const lastBlock =
            contentLeft > maxBlockSize ? 0 : !(RAND(seed) & 3);
        ...
        if (lastBlock) {
            ...
            if (contentLeft > 0 && (RAND(seed) & 7)) {
                ...
                blockContentSize =
                    RAND(seed) % (MIN(maxBlockSize, contentLeft) + 1);
...
static void
writeChecksum(frame_t *frame)
{
    /* write checksum so implementations can verify their output */
    U64 digest =
        XXH64(frame->srcStart, (BYTE *)frame->src - (BYTE *)frame->srcStart, 0);
...
static void
outputBuffer(const void *buf, size_t size, const char *const path)
{
 ...
 */
static U32
generateCompressedBlock(U32 seed, frame_t *frame, dictInfo info)
{
    size_t blockContentSize;
    int blockWritten = 0;
    BYTE *op;
    ...
    while (!blockWritten) {
        ...
        /* generate window size */
        {
            ...
            frame->header.windowSize +=
                (frame->header.windowSize / 8) * mantissa;
        }

        /* generate content size */
        {
            size_t const maxBlockSize =
                MIN(g_maxBlockSize, frame->header.windowSize);
            if (RAND(&seed) & 15) {
        ...
        if (cSize >=
            blockContentSize) { /* compressed size must be strictly smaller than
                                   decompressed size :
...
static U32
generateFrame(U32 seed, frame_t *fr, dictInfo info)
{
...
static int
genRandomDict(U32 dictID, U32 seed, size_t dictSize, BYTE *fullDict)
{
...

    {
        ...
        dictWriteSize = ZDICT_finalizeDictionary(
            fullDict, dictSize, dictContent, dictContentSize, samples,
            sampleSizes, numSamples, zdictParams);
...
static dictInfo
initDictInfo(int useDict, size_t dictContentSize, BYTE *dictContent, U32 dictID)
{
...
static size_t
testDecodeSimple(frame_t *fr)
{
    /* test decoding the generated data with the simple API */
    size_t const ret = ZSTD_decompress(
        DECOMPRESSED_BUFFER, MAX_DECOMPRESSED_SIZE, fr->dataStart,
        (BYTE *)fr->data - (BYTE *)fr->dataStart);
...
static size_t
testDecodeStreaming(frame_t *fr)
{
    ...
    while (1) {
        ...
        out.size +=
            MIN(ZSTD_DStreamOutSize(), MAX_DECOMPRESSED_SIZE - out.size);
    }
...
static size_t
testDecodeWithDict(U32 seed, genType_e genType)
{
    /* create variables */
    size_t const dictSize =
        RAND(&seed) % (10 << 20) + ZDICT_DICTSIZE_MIN + ZDICT_CONTENTSIZE_MIN;
    ...
    if (genRandomDict(dictID, seed, dictSize,
                      fullDict)) { /* return 0 on success */
...

    {
        ...
        if (genType == gt_frame) {
            ...
            ret = ZSTD_decompress_usingDict(
                dctx, DECOMPRESSED_BUFFER, MAX_DECOMPRESSED_SIZE, fr.dataStart,
                (BYTE *)fr.data - (BYTE *)fr.dataStart, fullDict, dictSize);
        } else {
            /* Test block */
            ...
            ret = ZSTD_decompressBlock(dctx, DECOMPRESSED_BUFFER,
                                       MAX_DECOMPRESSED_SIZE, fr.dataStart,
                                       (BYTE *)fr.data - (BYTE *)fr.dataStart);
        }
        ZSTD_freeDCtx(dctx);
...
runFrameTest(U32 *seed)
{
    ...
    {
        size_t const r =
            testDecodeWithDict(*seed, gt_frame); /* avoid big dictionaries */
        if (ZSTD_isError(r)) {
...
static int
runTestMode(U32 seed, unsigned numFiles, unsigned const testDurationS,
            genType_e genType)
{
    unsigned fnum;
    ...
    for (fnum = 0;
         fnum < numFiles || UTIL_clockSpanMicro(startClock) < maxClockSpan;
         fnum++) {
        if (fnum < numFiles)
        ...
        {
            int const ret = (genType == gt_frame) ? runFrameTest(&seed)
                                                  : runBlockTest(&seed);
            if (ret)
                return ret;
        }
...
static int
generateFile(U32 seed, const char *const path, const char *const origPath,
             genType_e genType)
{
    frame_t fr;
    ...
    if (origPath) {
        outputBuffer(fr.srcStart, (BYTE *)fr.src - (BYTE *)fr.srcStart,
                     origPath);
    }
    return 0;
}
...
static int
generateCorpus(U32 seed, unsigned numFiles, const char *const path,
               const char *const origPath, genType_e genType)
{
    ...
    for (fnum = 0; fnum < numFiles; fnum++) {
        ...
        if (snprintf(outPath, MAX_PATH, "%s/z%06u.zst", path, fnum) + 1 >
            MAX_PATH) {
            DISPLAY("Error: path too long\n");
            return 1;
        }
        outputBuffer(fr.dataStart, (BYTE *)fr.data - (BYTE *)fr.dataStart,
                     outPath);

        if (origPath) {
            if (snprintf(outPath, MAX_PATH, "%s/z%06u", origPath, fnum) + 1 >
                MAX_PATH) {
                DISPLAY("Error: path too long\n");
                return 1;
            }
...
static int
generateCorpusWithDict(U32 seed, unsigned numFiles, const char *const path,
    ...
    /* generate random compressed/decompressed files */
    {
        ...
        for (fnum = 0; fnum < MAX(numFiles, 1); fnum++) {
            ...
            DISPLAYUPDATE("\r%u/%u        ", fnum, numFiles);
            {
                ...
                BYTE *const dictContent = fullDict + headerSize;
                dictInfo const info =
                    initDictInfo(1, dictContentSize, dictContent, dictID);
                if (genType == gt_frame) {
                    seed = generateFrame(seed, &fr, info);
                } else {
                    seed = generateCompressedBlock(seed, &fr, info);
            ...
            if (numFiles != 0) {
                if (snprintf(outPath, MAX_PATH, "%s/z%06u.zst", path, fnum) +
                        1 >
                    MAX_PATH) {
                    DISPLAY("Error: path too long\n");
                    errorDetected = 1;
                ...
                outputBuffer(fr.dataStart,
                             (BYTE *)fr.data - (BYTE *)fr.dataStart, outPath);

                if (origPath) {
                    if (snprintf(outPath, MAX_PATH, "%s/z%06u", origPath,
                                 fnum) +
                            1 >
                        MAX_PATH) {
                        DISPLAY("Error: path too long\n");
                    ...
                    outputBuffer(fr.srcStart,
                                 (BYTE *)fr.src - (BYTE *)fr.srcStart, outPath);
                }
            } else {
                outputBuffer(fr.dataStart,
                             (BYTE *)fr.data - (BYTE *)fr.dataStart, path);
                if (origPath) {
                    outputBuffer(fr.srcStart,
                                 (BYTE *)fr.src - (BYTE *)fr.srcStart,
                                 origPath);
                }
            }
        }
...
usage(const char *programName)
{
    ...
    DISPLAY(
        "                in multiple files mode this should be a directory\n");
    DISPLAY(
        " -o<path> : select path to output original file (default:no "
        "output)\n");
    DISPLAY(
        "                in multiple files mode this should be a directory\n");
    ...
    DISPLAY(
        " -t       : activate test mode (test files against libzstd instead of "
        "outputting them)\n");
...
static void
advancedUsage(const char *programName)
{
    usage(programName);
    DISPLAY("\n");
    DISPLAY("Advanced arguments        :\n");
    DISPLAY(
        " --content-size           : always include the content size in the "
        "frame header\n");
    DISPLAY(
        " --use-dict=#             : include a dictionary used to decompress "
        "the corpus\n");
    DISPLAY(
        " --gen-blocks             : generate raw compressed blocks without "
        "block/frame headers\n");
    DISPLAY(
        " --max-block-size-log=#   : max block size log, must be in range [2, "
        "17]\n");
    DISPLAY(
        " --max-content-size-log=# : max content size log, must be <= 20\n");
            ...
            while (*argument != 0) {
                ...
                case '-':
                    ...
                    if (strcmp(argument, "content-size") == 0) {
                        ...
                        genType = gt_block;
                    } else if (longCommandWArg(&argument,
                                               "max-block-size-log=")) {
