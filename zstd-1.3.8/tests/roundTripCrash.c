...
#define CHECK_Z(f)                                                         \
    {                                                                      \
        size_t const err = f;                                              \
        if (ZSTD_isError(err)) {                                           \
            fprintf(stderr, "Error=> %s: %s", #f, ZSTD_getErrorName(err)); \
            crash(1);                                                      \
        }                                                                  \
    }
...
static size_t
roundTripTest(void *resultBuff, size_t resultBuffCapacity, void *compressedBuff,
              size_t compressedBuffCapacity, const void *srcBuff,
              size_t srcBuffSize)
{
    ...
    size_t const cSize = ZSTD_compress(compressedBuff, compressedBuffCapacity,
                                       srcBuff, srcBuffSize, cLevel);
    if (ZSTD_isError(cSize)) {
        fprintf(stderr, "Compression error : %s \n", ZSTD_getErrorName(cSize));
        return cSize;
    }
    return ZSTD_decompress(resultBuff, resultBuffCapacity, compressedBuff,
                           cSize);
}
 ...
 */
static size_t
cctxParamRoundTripTest(void *resultBuff, size_t resultBuffCapacity,
                       ...
                       const void *srcBuff, size_t srcBuffSize)
{
    ...
    /* Set parameters */
    CHECK_Z(ZSTD_CCtxParam_setParameter(cctxParams, ZSTD_c_compressionLevel,
                                        cLevel));
...
static size_t
checkBuffers(const void *buff1, const void *buff2, size_t buffSize)
{
...
static void
roundTripCheck(const void *srcBuff, size_t srcBuffSize, int testCCtxParams)
{
...

    {
        size_t const result =
            testCCtxParams
                ? cctxParamRoundTripTest(rBuff, cBuffSize, cBuff, cBuffSize,
                                         srcBuff, srcBuffSize)
                : roundTripTest(rBuff, cBuffSize, cBuff, cBuffSize, srcBuff,
                                srcBuffSize);
...
static size_t
getFileSize(const char *infilename)
{
    int r;
#if defined(_MSC_VER)
...
static int
isDirectory(const char *infilename)
{
    int r;
#if defined(_MSC_VER)
    struct _stat64 statbuf;
...
static void
loadFile(void *buffer, const char *fileName, size_t fileSize)
{
...
static void
fileCheck(const char *fileName, int testCCtxParams)
{
