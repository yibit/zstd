...
#define WELCOME_MESSAGE                                    \
    "*** %s %s %i-bits, by %s ***\n", PROGRAM_DESCRIPTION, \
        ZSTD_VERSION_STRING, (int)(sizeof(void *) * 8), AUTHOR

#define TIMELOOP_NANOSEC (1 * 1000000000ULL) /* 1 second */
#define NB_LEVELS_TRACKED \
    22 /* ensured being >= ZSTD_maxCLevel() in BMK_init_level_constraints() */

static const size_t maxMemory =
    (sizeof(size_t) == 4) ? (2 GB - 64 MB)
...
#define CHECKTIME(r)                                  \
    {                                                 \
        if (BMK_timeSpan_s(g_time) > g_timeLimit_s) { \
            DEBUGOUTPUT("Time Limit Reached\n");      \
            return r;                                 \
        }                                             \
    }
...
static const char *g_stratName[ZSTD_STRATEGY_MAX + 1] = {
    "(none)       ", "ZSTD_fast    ", "ZSTD_dfast   ", "ZSTD_greedy  ",
    "ZSTD_lazy    ", "ZSTD_lazy2   ", "ZSTD_btlazy2 ", "ZSTD_btopt   ",
    "ZSTD_btultra ", "ZSTD_btultra2"};

static const U32 tlen_table[TLEN_RANGE] = {
...
static const U32 mintable[NUM_PARAMS] = {
    ...
    ZSTD_SEARCHLOG_MIN, ZSTD_MINMATCH_MIN, ZSTD_TARGETLENGTH_MIN,
    ZSTD_STRATEGY_MIN,  FADT_MIN};

/* maximum value of parameters */
static const U32 maxtable[NUM_PARAMS] = {
    ZSTD_WINDOWLOG_MAX, ZSTD_CHAINLOG_MAX, ZSTD_HASHLOG_MAX,
    ZSTD_SEARCHLOG_MAX, ZSTD_MINMATCH_MAX, ZSTD_TARGETLENGTH_MAX,
    ZSTD_STRATEGY_MAX,  FADT_MAX};

/* # of values parameters can take on */
static const U32 rangetable[NUM_PARAMS] = {WLOG_RANGE, CLOG_RANGE, HLOG_RANGE,
                                           SLOG_RANGE, MML_RANGE,  TLEN_RANGE,
                                           STRT_RANGE, FADT_RANGE};

/* ZSTD_cctxSetParameter() index to set */
static const ZSTD_cParameter cctxSetParamTable[NUM_PARAMS] = {
    ZSTD_c_windowLog, ZSTD_c_chainLog,       ZSTD_c_hashLog,
    ZSTD_c_searchLog, ZSTD_c_minMatch,       ZSTD_c_targetLength,
    ZSTD_c_strategy,  ZSTD_c_forceAttachDict};

/* names of parameters */
static const char *g_paramNames[NUM_PARAMS] = {
    "windowLog", "chainLog",     "hashLog",  "searchLog",
    "minMatch",  "targetLength", "strategy", "forceAttachDict"};
...
static U32
rangeMap(varInds_t param, int ind)
{
...
/* inverse of rangeMap */
static int
invRangeMap(varInds_t param, U32 value)
{
...
/* display of params */
static void
displayParamVal(FILE *f, varInds_t param, unsigned value, int width)
{
    switch (param) {
    case wlog_ind:
    case clog_ind:
...
static UTIL_time_t g_time; /* to be used to compare solution finding speeds to
                              compare to original */
...
static BYTE g_silenceParams
    [NUM_PARAMS]; /* can selectively silence some params when displaying them */
...
/* For optimizer */
static paramValues_t g_params; /* Initialized at the beginning of main w/
                                  emptyParams() function */
static double g_ratioMultiplier = 5.;
static U32 g_strictness =
    PARAM_UNSET; /* range 1 - 100, measure of how strict  */
static BMK_benchResult_t g_lvltarget;

typedef enum { directMap, xxhashMap, noMemo } memoTableType_t;
...
static winner_ll_node
    *g_winners; /* linked list sorted ascending by cSize & cSpeed */
...
sanitizeParams(paramValues_t params)
{
    ...
    if ((params.vals[strt_ind] < ZSTD_btopt) &&
        (params.vals[strt_ind] != ZSTD_fast))
...
adjustParams(paramValues_t p, const size_t maxBlockSize, const size_t dictSize)
{
    ...
    varInds_t i;
    p = cParamsToPVals(
        ZSTD_adjustCParams(pvalsToCParams(p), maxBlockSize, dictSize));
    if (!dictSize) {
        p.vals[fadt_ind] = 0;
    }
...
static size_t
BMK_findMaxMem(U64 requiredMem)
{
...
static int
paramValid(const paramValues_t paramTarget)
{
    U32 i;
...
{
    ptr->vals[paramIndex] = rangeMap(
        paramIndex, invRangeMap(paramIndex, ptr->vals[paramIndex]) + amt);
}
...
paramVariation(paramValues_t *ptr, memoTable_t *mtAll, const U32 nbChanges)
{
    ...
    while (!validated) {
        ...
        for (i = 0; i < nbChanges; i++) {
            const U32 changeID =
                (U32)FUZ_rand(&g_rand) % (mtAll[p.vals[strt_ind]].varLen << 1);
...
static int
feasible(const BMK_benchResult_t results, const constraint_t target)
{
    return (results.cSpeed >= target.cSpeed) &&
           (results.dSpeed >= target.dSpeed) && (results.cMem <= target.cMem) &&
           (!g_optmode || results.cSize <= g_lvltarget.cSize);
}

/* hill climbing value for part 1 */
/* Scoring here is a linear reward for all set constraints normalized between 0
 * to 1
 * (with 0 at 0 and 1 being fully fulfilling the constraint), summed with a
 * logarithmic
 * bonus to exceeding the constraint value. We also give linear ratio for
 * compression ratio.
 * The constant factors are experimental.
 */
static double
resultScore(const BMK_benchResult_t res, const size_t srcSize,
            const constraint_t target)
{
    ...
    double ret;
    if (target.cSpeed) {
        cs = res.cSpeed / (double)target.cSpeed;
    }
    if (target.dSpeed) {
        ds = res.dSpeed / (double)target.dSpeed;
    }
    if (target.cMem != (U32)-1) {
        cm = (double)target.cMem / res.cMem;
    }
...
/* calculates normalized squared euclidean distance of result1 if it is in the
 * first quadrant relative to lvlRes */
static double
...
static int
compareResultLT(const BMK_benchResult_t result1,
                const BMK_benchResult_t result2, const constraint_t target,
                size_t srcSize)
{
    if (feasible(result1, target) && feasible(result2, target)) {
        if (g_optmode) {
            return resultDistLvl(result1, g_lvltarget) <
                   resultDistLvl(result2, g_lvltarget);
        } else {
            return (result1.cSize > result2.cSize) ||
                   (result1.cSize == result2.cSize &&
                    result2.cSpeed > result1.cSpeed) ||
                   (result1.cSize == result2.cSize &&
                    result2.cSpeed == result1.cSpeed &&
                    result2.dSpeed > result1.dSpeed);
        }
    }
    return feasible(result2, target) ||
           (!feasible(result1, target) &&
            (resultScore(result1, srcSize, target) <
             resultScore(result2, srcSize, target)));
}
...
static void
optimizerAdjustInput(paramValues_t *pc, const size_t maxBlockSize)
{
    varInds_t v;
    for (v = 0; v < NUM_PARAMS; v++) {
        if (pc->vals[v] != PARAM_UNSET) {
            ...
            if (newval != pc->vals[v]) {
                pc->vals[v] = newval;
                DISPLAY(
                    "Warning: parameter %s not in valid range, adjusting to ",
                    g_paramNames[v]);
                displayParamVal(stderr, v, newval, 0);
                DISPLAY("\n");
            }
        }
    }
    ...
    if (pc->vals[wlog_ind] != PARAM_UNSET) {
        U32 sshb =
            maxBlockSize > 1 ? ZSTD_highbit32((U32)(maxBlockSize - 1)) + 1 : 1;
        ...
        if (maxBlockSize < (1ULL << 31) && sshb + 1 < pc->vals[wlog_ind]) {
            ...
            if (adjust != pc->vals[wlog_ind]) {
                ...
                DISPLAY(
                    "Warning: windowLog larger than src/block size, adjusted "
                    "to %u\n",
    ...
    if (pc->vals[wlog_ind] != PARAM_UNSET &&
        pc->vals[clog_ind] != PARAM_UNSET) {
        U32 maxclog;
        if (pc->vals[strt_ind] == PARAM_UNSET ||
            pc->vals[strt_ind] >= (U32)ZSTD_btlazy2) {
        ...
        if (pc->vals[clog_ind] > maxclog) {
            ...
            DISPLAY(
                "Warning: chainlog too much larger than windowLog size, "
                "adjusted to %u\n",
    ...
    if (pc->vals[wlog_ind] != PARAM_UNSET &&
        pc->vals[hlog_ind] != PARAM_UNSET) {
        if (pc->vals[wlog_ind] + 1 < pc->vals[hlog_ind]) {
            pc->vals[hlog_ind] = pc->vals[wlog_ind] + 1;
            DISPLAY(
                "Warning: hashlog too much larger than windowLog size, "
                "adjusted to %u\n",
                (unsigned)pc->vals[hlog_ind]);
        }
    }

    if (pc->vals[slog_ind] != PARAM_UNSET &&
        pc->vals[clog_ind] != PARAM_UNSET) {
...
static int
redundantParams(const paramValues_t paramValues, const constraint_t target,
                const size_t maxBlockSize)
{
    return (ZSTD_estimateCStreamSize_usingCParams(pvalsToCParams(paramValues)) >
            (size_t)target.cMem) /* Uses too much memory */
           || ((1ULL << (paramValues.vals[wlog_ind] - 1)) >= maxBlockSize &&
               paramValues.vals[wlog_ind] !=
                   mintable[wlog_ind]) /* wlog too much bigger than src size */
           || (paramValues.vals[clog_ind] >
               (paramValues.vals[wlog_ind] +
                (paramValues.vals[strt_ind] >
                 ZSTD_btlazy2))) /* chainLog larger than windowLog*/
           || (paramValues.vals[slog_ind] >
               paramValues.vals[clog_ind]) /* searchLog larger than chainLog */
           || (paramValues.vals[hlog_ind] >
               paramValues.vals[wlog_ind] +
...
BMK_paramValues_into_commandLine(FILE *f, const paramValues_t params)
{
    ...
    for (v = 0; v < NUM_PARAMS; v++) {
        ...
        if (v == strt_ind) {
            fprintf(f, "%u", (unsigned)params.vals[v]);
        } else {
            displayParamVal(f, v, params.vals[v], 0);
        }
        first = 0;
    }
    fprintf(f, "\n");
...
BMK_displayOneResult(FILE *f, winnerInfo_t res, const size_t srcSize)
{
    ...
    {
        double const ratio =
            res.result.cSize ? (double)srcSize / res.result.cSize : 0;
        ...
        fprintf(f, " },     /* R:%5.3f at %5.1f MB/s - %5.1f MB/s */\n", ratio,
                cSpeedMBps, dSpeedMBps);
    }
}

/* Writes to f the results of a parameter benchmark */
/* when used with --optimize, will only print results better than previously
 * discovered */
static void
BMK_printWinner(FILE *f, const int cLevel, const BMK_benchResult_t result,
                const paramValues_t params, const size_t srcSize)
{
    ...
    if (TIMED) {
        ...
        fprintf(f, "%1lu:%2lu:%05.2f - ", (unsigned long)minutes / 60,
...
static void
BMK_printWinnerOpt(FILE *f, const U32 cLevel, const BMK_benchResult_t result,
                   const paramValues_t params,
                   const constraint_t targetConstraints, const size_t srcSize)
{
    /* global winner used for constraints */
    /* cSize, cSpeed, dSpeed, cMem */
    static winnerInfo_t g_winner = {
        {(size_t)-1LL, 0, 0, (size_t)-1LL},
        {{PARAM_UNSET, PARAM_UNSET, PARAM_UNSET, PARAM_UNSET, PARAM_UNSET,
          PARAM_UNSET, PARAM_UNSET, PARAM_UNSET}}};
    if (DEBUG ||
        compareResultLT(g_winner.result, result, targetConstraints, srcSize) ||
        g_displayLevel >= 4) {
        if (DEBUG && compareResultLT(g_winner.result, result, targetConstraints,
                                     srcSize)) {
            DISPLAY("New Winner: \n");
        }
        ...
        if (compareResultLT(g_winner.result, result, targetConstraints,
                            srcSize)) {
            if (g_displayLevel >= 1) {
                BMK_paramValues_into_commandLine(f, params);
            }
    ...
    if (g_optmode && g_optimizer && (DEBUG || g_displayLevel == 3)) {
        ...
        fprintf(f, "Level Bounds: R: > %.3f AND C: < %.1f MB/s \n\n",
                (double)srcSize / g_lvltarget.cSize,
                (double)g_lvltarget.cSpeed / MB_UNIT);
 ...
 */
static void
BMK_print_cLevelEntry(FILE *f, const int cLevel, paramValues_t params,
                      const BMK_benchResult_t result, const size_t srcSize)
{
    ...
    /* print comment */
    {
        double const ratio = result.cSize ? (double)srcSize / result.cSize : 0;
 ...
 * f must exist, be already opened, and be seekable.
 * winners must be a table of NB_LEVELS_TRACKED+1 elements winnerInfo_t, all
 * entries presumed initialized
 * this function cannot error.
 */
static void
BMK_print_cLevelTable(FILE *f, const winnerInfo_t *winners,
                      const size_t srcSize)
{
    ...
    for (cLevel = 0; cLevel <= NB_LEVELS_TRACKED; cLevel++)
        BMK_print_cLevelEntry(f, cLevel, winners[cLevel].params,
 ...
 */
static void
BMK_saveAndPrint_cLevelTable(FILE *const f, const winnerInfo_t *winners,
                             const size_t srcSize)
{
    fseek(f, 0, SEEK_SET);
...
local_initCCtx(void *payload)
{
    ...
    for (i = 0; i < NUM_PARAMS; i++) {
        if (ag->comprParams->vals[i] != PARAM_UNSET)
            ZSTD_CCtx_setParameter(ag->cctx, cctxSetParamTable[i],
                                   ag->comprParams->vals[i]);
    }
...
static size_t
local_defaultCompress(const void *srcBuffer, size_t srcSize, void *dstBuffer,
                      size_t dstSize, void *addArgs)
{
    ZSTD_CCtx *cctx = (ZSTD_CCtx *)addArgs;
    assert(dstSize ==
           ZSTD_compressBound(srcSize)); /* specific to this version, which is
                                            only used in paramgrill */
    return ZSTD_compress2(cctx, dstBuffer, dstSize, srcBuffer, srcSize);
}

/* additional argument is just the context */
static size_t
local_defaultDecompress(const void *srcBuffer, size_t srcSize, void *dstBuffer,
                        size_t dstSize, void *addArgs)
{
...
static void
freeNonSrcBuffers(const buffers_t b)
{
    free(b.srcPtrs);
    free(b.srcSizes);
...
static int
createBuffersFromMemory(buffers_t *buff, void *srcBuffer, const size_t nbFiles,
                        const size_t *fileSizes)
{
    ...
    maxNbBlocks =
        (U32)((buff->srcSize + (blockSize - 1)) / blockSize) + (U32)nbFiles;
    ...
    if (!buff->srcPtrs || !buff->srcSizes || !buff->dstPtrs ||
        !buff->dstCapacities || !buff->dstSizes || !buff->resPtrs ||
        !buff->resSizes) {
        DISPLAY("alloc error\n");
    ...
    buff->dstPtrs[0] =
        malloc(ZSTD_compressBound(buff->srcSize) + (maxNbBlocks * 1024));
    ...
    for (n = 0; n < nbFiles; n++) {
        ...
        if (fileSizes[n] > 0) {
            buff->srcSizes[blockNb - 1] = ((fileSizes[n] - 1) % blockSize) + 1;
        }
        pos = pos_end;
    }
    ...
    for (n = 1; n < blockNb; n++) {
        buff->dstPtrs[n] =
            ((char *)buff->dstPtrs[n - 1]) + buff->dstCapacities[n - 1];
        buff->resPtrs[n] =
            ((char *)buff->resPtrs[n - 1]) + buff->resSizes[n - 1];
...
static int
createBuffers(buffers_t *buff, const char *const *const fileNamesTable,
              size_t nbFiles)
{
    size_t pos = 0;
    size_t n;
    ...
    size_t benchedSize =
        MIN(BMK_findMaxMem(totalSizeToLoad * 3) / 3, totalSizeToLoad);
...
static void
freeContexts(const contexts_t ctx)
{
    free(ctx.dictBuffer);
...
static int
createContexts(contexts_t *ctx, const char *dictFileName)
{
    FILE *f;
    size_t readSize;
...
static size_t
sanitizeVarArray(varInds_t *varNew, const size_t varLength,
                 const varInds_t *varArray, const ZSTD_strategy strat)
{
    size_t i, j = 0;
    for (i = 0; i < varLength; i++) {
        if (!((varArray[i] == clog_ind && strat == ZSTD_fast) ||
              ...
              (varArray[i] == tlen_ind && strat < ZSTD_btopt &&
               strat != ZSTD_fast))) {
            varNew[j] = varArray[i];
            j++;
        }
...
/* pass in using dict. */
static size_t
variableParams(const paramValues_t paramConstraints, varInds_t *res,
...
static size_t
memoTableLen(const varInds_t *varyParams, const size_t varyLen)
{
    size_t arrayLen = 1;
    size_t i;
    for (i = 0; i < varyLen; i++) {
...
static unsigned
memoTableIndDirect(const paramValues_t *ptr, const varInds_t *varyParams,
                   const size_t varyLen)
{
    ...
    for (i = 0; i < varyLen; i++) {
        ...
        ind *= rangetable[v];
        ind += (unsigned)invRangeMap(v, ptr->vals[v]);
    }
    return ind;
}

static size_t
memoTableGet(const memoTable_t *memoTableArray, const paramValues_t p)
{
    ...
    case xxhashMap:
        return mt.table[(XXH64(&p.vals, sizeof(U32) * NUM_PARAMS, 0) >> 3) %
                        mt.tableLen];
    case noMemo:
        return 0;
    }
...
static void
memoTableSet(const memoTable_t *memoTableArray, const paramValues_t p,
             const BYTE value)
{
    const memoTable_t mt = memoTableArray[p.vals[strt_ind]];
    switch (mt.tableType) {
    case directMap:
        mt.table[memoTableIndDirect(&p, mt.varArray, mt.varLen)] = value;
        break;
    case xxhashMap:
        mt.table[(XXH64(&p.vals, sizeof(U32) * NUM_PARAMS, 0) >> 3) %
                 mt.tableLen] = value;
        break;
    case noMemo:
        break;
...
static void
freeMemoTableArray(memoTable_t *const mtAll)
{
    int i;
    if (mtAll == NULL) {
        return;
...
static memoTable_t *
createMemoTableArray(const paramValues_t p, const varInds_t *const varyParams,
                     const size_t varyLen, const U32 memoTableLog)
{
    memoTable_t *const mtAll =
        (memoTable_t *)calloc(sizeof(memoTable_t), (ZSTD_STRATEGY_MAX + 1));
    ...
    for (i = 1; i <= (int)ZSTD_STRATEGY_MAX; i++) {
        mtAll[i].varLen =
            sanitizeVarArray(mtAll[i].varArray, varyLen, varyParams, i);
    }
    ...
    for (i = stratMin; i <= stratMax; i++) {
        ...
        if (memoTableLog != PARAM_UNSET &&
            mtl > (1ULL
                   << memoTableLog)) { /* use hash table */ /* provide some
                                                               option to only
                                                               use hash tables?
                                                               */
...
/* specifiy strategy */
static void
randomConstrainedParams(paramValues_t *pc, const memoTable_t *memoTableArray,
                        const ZSTD_strategy st)
{
    ...
    for (j = 0; j < mt.tableLen; j++) {
        ...
        if (!(memoTableGet(memoTableArray, *pc)))
...
static void
display_params_tested(paramValues_t cParams)
{
    varInds_t vi;
...
/* Replicate functionality of benchMemAdvanced, but with pre-split src / dst
 * buffers */
/* The purpose is so that sufficient information is returned so that a
 * decompression call to benchMemInvertible is possible */
/* BMK_benchMemAdvanced(srcBuffer,srcSize, dstBuffer, dstSize, fileSizes,
 * nbFiles, 0, &cParams, dictBuffer, dictSize, ctx, dctx, 0, "File", &adv); */
/* nbSeconds used in same way as in BMK_advancedParams_t */
/* if in decodeOnly, then srcPtr's will be compressed blocks, and
 * uncompressedBlocks will be written to dstPtrs */
...
BMK_benchMemInvertible(buffers_t buf, contexts_t ctx, int cLevel,
                       const paramValues_t *comprParams, BMK_mode_t mode,
                       unsigned nbSeconds)
{
    U32 i;
    ...
    /* Bench */
    {
        ...
        BMK_timedFnState_t *timeStateCompress =
            BMK_createTimedFnState(nbSeconds * 1000, 1000);
        ...
        while (!compressionCompleted) {
            ...
            }
            {
                ...
                bResult.cSpeed =
                    (srcSize * TIMELOOP_NANOSEC) / rResult.nanoSecPerRun;
    ...
    /* Bench */
    bResult.cMem =
        (1 << (comprParams->vals[wlog_ind])) + ZSTD_sizeof_CCtx(cctx);
...
static int
BMK_benchParam(BMK_benchResult_t *resultPtr, buffers_t buf, contexts_t ctx,
               paramValues_t cParams)
{
    BMK_benchOutcome_t const outcome =
        BMK_benchMemInvertible(buf, ctx, BASE_CLEVEL, &cParams, BMK_both, 3);
...
/* Benchmarking which stops when we are sufficiently sure the solution is
 * infeasible / worse than the winner */
#define VARIANCE 1.2
static int
allBench(BMK_benchResult_t *resultPtr, const buffers_t buf,
         const contexts_t ctx, const paramValues_t cParams,
         const constraint_t target, BMK_benchResult_t *winnerResult, int feas)
{
    ...
    BMK_benchOutcome_t const outcome =
        BMK_benchMemInvertible(buf, ctx, BASE_CLEVEL, &cParams, BMK_both, 2);
    if (!BMK_isSuccessful_benchOutcome(outcome)) {
    ...
    *resultPtr = benchres;
    ...
    if (benchres.cSpeed) {
        U64 const loopDurationC =
            (((U64)buf.srcSize * TIMELOOP_NANOSEC) / benchres.cSpeed);
        uncertaintyConstantC =
            ((loopDurationC + (double)(2 * g_clockGranularity)) /
             loopDurationC);
    }

    if (benchres.dSpeed) {
        U64 const loopDurationD =
            (((U64)buf.srcSize * TIMELOOP_NANOSEC) / benchres.dSpeed);
        uncertaintyConstantD =
            ((loopDurationD + (double)(2 * g_clockGranularity)) /
             loopDurationD);
    }
    ...
    if ((!feas && (resultScore(benchres, buf.srcSize, target) >
                   resultScore(*winnerResult, buf.srcSize, target))) ||
        (feas &&
...
/* Memoized benchmarking, won't benchmark anything which has already been
 * benchmarked before. */
static int
benchMemo(BMK_benchResult_t *resultPtr, const buffers_t buf,
          const contexts_t ctx, const paramValues_t cParams,
          const constraint_t target, BMK_benchResult_t *winnerResult,
          memoTable_t *const memoTableArray, const int feas)
{
    ...
    BMK_printWinnerOpt(stdout, CUSTOM_LEVEL, *resultPtr, cParams, target,
                       buf.srcSize);

    if (res == BETTER_RESULT || feas) {
        memoTableSet(memoTableArray, cParams,
...
static void
BMK_init_level_constraints(int bytePerSec_level1)
{
    ...
    /* establish speed objectives (relative to level 1) */
    {
        ...
        for (l = 2; l <= NB_LEVELS_TRACKED; l++) {
            g_level_constraint[l].cSpeed_min =
                (g_level_constraint[l - 1].cSpeed_min * 49) / 64;
            g_level_constraint[l].dSpeed_min = 0.;
            g_level_constraint[l].windowLog_max =
                (l < 20) ? 23 : l + 5; /* only --ultra levels >= 20 can use
                                          windowlog > 23 */
...
static int
BMK_seed(winnerInfo_t *winners, const paramValues_t params, const buffers_t buf,
         const contexts_t ctx)
{
    ...
    for (cLevel = 1; cLevel <= NB_LEVELS_TRACKED; cLevel++) {
        ...
        if (winners[cLevel].result.cSize == 0) {
            ...
            BMK_print_cLevelEntry(stdout, cLevel, params, testResult,
                                  buf.srcSize);
            better = 1;
            continue;
        }

        if ((double)testResult.cSize <=
            ((double)winners[cLevel].result.cSize * (1. + (0.02 / cLevel)))) {
            ...
            size_t O_DMemUsed =
                (1 << winners[cLevel].params.vals[wlog_ind]) + (16 KB);
            double W_DMemUsed_note =
                W_ratioNote * (40 + 9 * cLevel) - log((double)W_DMemUsed);
            double O_DMemUsed_note =
                O_ratioNote * (40 + 9 * cLevel) - log((double)O_DMemUsed);

            size_t W_CMemUsed =
                (1 << params.vals[wlog_ind]) +
                ZSTD_estimateCCtxSize_usingCParams(pvalsToCParams(params));
            size_t O_CMemUsed = (1 << winners[cLevel].params.vals[wlog_ind]) +
                                ZSTD_estimateCCtxSize_usingCParams(
                                    pvalsToCParams(winners[cLevel].params));
            double W_CMemUsed_note =
                W_ratioNote * (50 + 13 * cLevel) - log((double)W_CMemUsed);
            double O_CMemUsed_note =
                O_ratioNote * (50 + 13 * cLevel) - log((double)O_CMemUsed);

            double W_CSpeed_note =
                W_ratioNote * (30 + 10 * cLevel) + log(testResult.cSpeed);
            double O_CSpeed_note = O_ratioNote * (30 + 10 * cLevel) +
                                   log(winners[cLevel].result.cSpeed);

            double W_DSpeed_note =
                W_ratioNote * (20 + 2 * cLevel) + log(testResult.dSpeed);
            double O_DSpeed_note = O_ratioNote * (20 + 2 * cLevel) +
                                   log(winners[cLevel].result.dSpeed);
            ...
            if (W_DMemUsed_note < O_DMemUsed_note) {
                ...
                if (W_ratio > O_ratio)
                    DISPLAYLEVEL(
                        3,
                        "Decompression Memory : %5.3f @ %4.1f MB  vs  %5.3f @ "
                        "%4.1f MB   : not enough for level %i\n",
                        W_ratio, (double)(W_DMemUsed) / 1024 / 1024, O_ratio,
                        (double)(O_DMemUsed) / 1024 / 1024, cLevel);
                continue;
            }
            if (W_CMemUsed_note < O_CMemUsed_note) {
                ...
                if (W_ratio > O_ratio)
                    DISPLAYLEVEL(
                        3,
                        "Compression Memory : %5.3f @ %4.1f MB  vs  %5.3f @ "
                        "%4.1f MB   : not enough for level %i\n",
                        W_ratio, (double)(W_CMemUsed) / 1024 / 1024, O_ratio,
                        (double)(O_CMemUsed) / 1024 / 1024, cLevel);
                continue;
            }
            if (W_CSpeed_note < O_CSpeed_note) {
                /* too large compression speed difference for the compression
                 * benefit */
                if (W_ratio > O_ratio)
                    DISPLAYLEVEL(
                        3,
                        "Compression Speed : %5.3f @ %4.1f MB/s  vs  %5.3f @ "
                        "%4.1f MB/s   : not enough for level %i\n",
                        W_ratio, (double)testResult.cSpeed / MB_UNIT, O_ratio,
                        (double)winners[cLevel].result.cSpeed / MB_UNIT,
                        cLevel);
                continue;
            }
            if (W_DSpeed_note < O_DSpeed_note) {
                /* too large decompression speed difference for the compression
                 * benefit */
                if (W_ratio > O_ratio)
                    DISPLAYLEVEL(
                        3,
                        "Decompression Speed : %5.3f @ %4.1f MB/s  vs  %5.3f @ "
                        "%4.1f MB/s   : not enough for level %i\n",
                        W_ratio, (double)testResult.dSpeed / MB_UNIT, O_ratio,
                        (double)winners[cLevel].result.dSpeed / MB_UNIT,
                        cLevel);
                continue;
            }
            ...
            if (W_ratio < O_ratio)
                DISPLAYLEVEL(3,
                             "Solution %4.3f selected over %4.3f at level %i, "
                             "due to better secondary statistics \n",
...
static void
playAround(FILE *f, winnerInfo_t *winners, paramValues_t p, const buffers_t buf,
           const contexts_t ctx)
{
    int nbVariations = 0;
    UTIL_time_t const clockStart = UTIL_getTime();
    ...
    while (UTIL_clockSpanMicro(clockStart) < g_maxVariationTime) {
        if (nbVariations++ > g_maxNbVariations)
            break;
...
static void
BMK_selectRandomStart(FILE *f, winnerInfo_t *winners, const buffers_t buf,
                      const contexts_t ctx)
{
    U32 const id = FUZ_rand(&g_rand) % (NB_LEVELS_TRACKED + 1);
    if ((id == 0) || (winners[id].params.vals[wlog_ind] == 0)) {
        /* use some random entry */
        paramValues_t const p = adjustParams(
            cParamsToPVals(pvalsToCParams(
                randomParams())), /* defaults nonCompression parameters */
            buf.srcSize, 0);
        playAround(f, winners, p, buf, ctx);
    } else {
 ...
 */
static void
BMK_generate_cLevelTable(const buffers_t buf, const contexts_t ctx)
{
    paramValues_t params;
    ...
    if (g_target) {
        ...
        paramValues_t const l1params =
            cParamsToPVals(ZSTD_getCParams(1, buf.maxBlockSize, ctx.dictSize));
        BMK_benchResult_t testResult;
...
benchOnce(const buffers_t buf, const contexts_t ctx, const int cLevel)
{
    ...
    g_params = adjustParams(
        overwriteParams(cParamsToPVals(ZSTD_getCParams(cLevel, buf.maxBlockSize,
                                                       ctx.dictSize)),
                        g_params),
        buf.maxBlockSize, ctx.dictSize);
...
static int
benchSample(double compressibility, int cLevel)
{
...
static int
benchFiles(const char **fileNamesTable, int nbFiles, const char *dictFileName,
           int cLevel)
{
    buffers_t buf;
    contexts_t ctx;
 ...
 * failing that, it progressively tries candidates further and further away (up
 * to #dim + 2)
 * if it finds a candidate exceeding winnerInfo, it will repeat. Otherwise, it
 * will stop the
 * current stage of hill climbing.
 * Each iteration of hill climbing proceeds in 2 'phases'. Phase 1 climbs
 * according to
 * the resultScore function, which is effectively a linear increase in reward
 * until it reaches
 * the constraint-satisfying value, it which point any excess results in only
 * logarithmic reward.
 * This aims to find some constraint-satisfying point.
 * Phase 2 optimizes in accordance with what the original function sets out to
 * maximize, with
 ...
 */
static winnerInfo_t
climbOnce(const constraint_t target, memoTable_t *mtAll, const buffers_t buf,
          const contexts_t ctx, const paramValues_t init)
{
    /*
...

    {
        ...
        while (better) {
            ...
            for (i = 0; i < varLen; i++) {
                for (offset = -1; offset <= 1; offset += 2) {
                    ...
                    paramVaryOnce(mtAll[cparam.vals[strt_ind]].varArray[i],
                                  offset, &candidateInfo.params);

                    if (paramValid(candidateInfo.params)) {
                        int res;
                        res =
                            benchMemo(&candidateInfo.result, buf, ctx,
                                      sanitizeParams(candidateInfo.params),
                                      target, &winnerInfo.result, mtAll, feas);
                        DEBUGOUTPUT("Res: %d\n", res);
                        if (res == BETTER_RESULT) { /* synonymous with better
                                                       when called w/
                                                       infeasibleBM */
                            winnerInfo = candidateInfo;
                            better = 1;
                            if (compareResultLT(bestFeasible1.result,
                                                winnerInfo.result, target,
                                                buf.srcSize)) {
            ...
            for (dist = 2; dist < varLen + 2;
                 dist++) { /* varLen is # dimensions */
                for (i = 0; i < (1 << varLen) / varLen + 2; i++) {
                    ...
                    res = benchMemo(&candidateInfo.result, buf, ctx,
                                    ...
                                    target, &winnerInfo.result, mtAll, feas);
                    DEBUGOUTPUT("Res: %d\n", res);
                    if (res == BETTER_RESULT) { /* synonymous with better in
                                                   this case*/
                        winnerInfo = candidateInfo;
                        better = 1;
                        if (compareResultLT(bestFeasible1.result,
            ...
            if (!better) { /* infeas -> feas -> stop */
                ...
                better = 1;
                winnerInfo =
                    bestFeasible1; /* note with change, bestFeasible may not
                                      necessarily be feasible, but if one has
                                      been benchmarked, it will be. */
...
/* flexible parameters: iterations of failed climbing (or if we do non-random,
   maybe this is when everything is close to visitied)
   weight more on visit for bad results, less on good results/more on later
   results / ones with more failures.
   allocate memoTable here.
 */
static winnerInfo_t
optimizeFixedStrategy(const buffers_t buf, const contexts_t ctx,
                      const constraint_t target, paramValues_t paramTarget,
                      const ZSTD_strategy strat, memoTable_t *memoTableArray,
                      const int tries)
{
    int i = 0;
    ...
    for (i = 0; i < tries; i++) {
        ...
        if (compareResultLT(winnerInfo.result, candidateInfo.result, target,
                            buf.srcSize)) {
            winnerInfo = candidateInfo;
            BMK_printWinnerOpt(stdout, CUSTOM_LEVEL, winnerInfo.result,
                               winnerInfo.params, target, buf.srcSize);
            i = 0;
            continue;
        }
        CHECKTIME(winnerInfo);
...
static int
nextStrategy(const int currentStrategy, const int bestStrategy)
{
 ...
 * paramTarget - parameter constraints (i.e. restriction search space to where
 * strategy = ZSTD_fast)
 * cLevel - compression level to exceed (all solutions must be > lvl in cSpeed +
 * ratio)
 */
...
optimizeForSize(const char *const *const fileNamesTable, const size_t nbFiles,
                const char *dictFileName, constraint_t target,
                paramValues_t paramTarget, const int cLevelOpt,
                const int cLevelRun, const U32 memoTableLog)
{
    varInds_t varArray[NUM_PARAMS];
    int ret = 0;
    const size_t varLen =
        variableParams(paramTarget, varArray, dictFileName != NULL);
    ...
    if (nbFiles == 1) {
        ...
        DISPLAYLEVEL(2, "Loading %lu Files...       \r",
    ...
    if (g_optmode) {
        winner.params = cParamsToPVals(
            ZSTD_getCParams(cLevelOpt, buf.maxBlockSize, ctx.dictSize));
        ...
        BMK_printWinnerOpt(stdout, cLevelOpt, winner.result, winner.params,
                           target, buf.srcSize);
    }
    ...
    if (g_singleRun) {
        ...
        g_params = adjustParams(
            overwriteParams(cParamsToPVals(ZSTD_getCParams(
                                cLevelRun, buf.maxBlockSize, ctx.dictSize)),
                            g_params),
            buf.maxBlockSize, ctx.dictSize);
        ...
        if (compareResultLT(winner.result, res, relaxTarget(target),
                            buf.srcSize)) {
            winner.result = res;
    ...
    if (target.cSpeed != 0) {
        DISPLAYLEVEL(2, " - limit compression speed %u MB/s",
                     (unsigned)(target.cSpeed >> 20));
    }
    if (target.dSpeed != 0) {
        DISPLAYLEVEL(2, " - limit decompression speed %u MB/s",
                     (unsigned)(target.dSpeed >> 20));
    }
    if (target.cMem != (U32)-1) {
        DISPLAYLEVEL(2, " - limit memory %u MB", (unsigned)(target.cMem >> 20));
    }
...

    {
        ...
        /* find best solution from default params */
        {
            ...
            if (paramTarget.vals[strt_ind] == PARAM_UNSET) {
                ...
                for (i = 1; i <= maxSeeds; i++) {
                    ...
                    CParams =
                        overwriteParams(cParamsToPVals(ZSTD_getCParams(
                                            i, buf.maxBlockSize, ctx.dictSize)),
                                        paramTarget);
                    ec = BMK_benchParam(&candidate, buf, ctx, CParams);
                    BMK_printWinnerOpt(stdout, i, candidate, CParams, target,
                                       buf.srcSize);

                    if (!ec &&
                        compareResultLT(winner.result, candidate,
                                        relaxTarget(target), buf.srcSize)) {
                        winner.result = candidate;
                        winner.params = CParams;
                    }

                    CHECKTIMEGT(ret, 0,
                                _displayCleanUp); /* if pass time limit, stop */
                    /* if the current params are too slow, just stop. */
                    if (target.cSpeed > candidate.cSpeed * 3 / 2) {
                        break;
                    }
                }

                BMK_printWinnerOpt(stdout, CUSTOM_LEVEL, winner.result,
                                   winner.params, target, buf.srcSize);
            }
        }
        ...
        /* start 'real' optimization */
        {
            ...
            if (paramTarget.vals[strt_ind] == PARAM_UNSET) {
                ...
                /* one iterations of hill climbing with the level-defined
                 * parameters. */
                {
                    winnerInfo_t const w1 =
                        climbOnce(target, allMT, buf, ctx, winner.params);
                    if (compareResultLT(winner.result, w1.result, target,
                                        buf.srcSize)) {
                        winner = w1;
                    }
                ...
                while (st && tries > 0) {
                    ...
                    wc = optimizeFixedStrategy(buf, ctx, target, paramBase, st,
                                               allMT, tries);

                    if (compareResultLT(winner.result, wc.result, target,
                                        buf.srcSize)) {
                        winner = wc;
                        tries = g_maxTries;
                        bestStrategy = st;
...
static double
readDoubleFromChar(const char **stringPtr)
{
    ...
    (*stringPtr)++;
    while ((**stringPtr >= '0') && (**stringPtr <= '9')) {
        result += (double)(**stringPtr - '0') / divide, divide *= 10,
            (*stringPtr)++;
    }
    return result;
}
...
usage(const char *exename)
{
    ...
    DISPLAY("Arguments :\n");
    DISPLAY(
        " file : path to the file used as reference (if none, generates a "
        "compressible sample)\n");
...
usage_advanced(void)
{
    ...
    DISPLAY(
        " -B#          : cut input into blocks of size # (default : single "
        "block) \n");
    DISPLAY(
        " --optimize=  : same as -O with more verbose syntax (see "
        "README.md)\n");
    DISPLAY(" -S           : Single run \n");
    DISPLAY(
        " --zstd       : Single run, parameter selection same as zstdcli \n");
    DISPLAY(
        " -P#          : generated sample compressibility (default : %.1f%%) "
        "\n",
        COMPRESSIBILITY_DEFAULT * 100);
    DISPLAY(
        " -t#          : Caps runtime of operation in seconds (default : %u "
        "seconds (%.1f hours)) \n",
...
static int
parse_params(const char **argptr, paramValues_t *pv)
{
    int matched = 0;
...
main(int argc, const char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (longCommandWArg(&argument, "--optimize=")) {
            g_optimizer = 1;
            for (;;) {
                if (parse_params(&argument, &paramTarget)) {
                    if (argument[0] == ',') {
                        argument++;
                        continue;
                    } else
                ...
                if (longCommandWArg(&argument, "level=") ||
                    longCommandWArg(&argument, "lvl=")) {
                    cLevelOpt = readU32FromChar(&argument);
                    g_optmode = 1;
                    if (argument[0] == ',') {
                        argument++;
                        continue;
                ...
                if (longCommandWArg(&argument, "speedForRatio=") ||
                    longCommandWArg(&argument, "speedRatio=")) {
                    g_ratioMultiplier = readDoubleFromChar(&argument);
                    if (argument[0] == ',') {
                        argument++;
                        continue;
                    } else
            ...
            g_singleRun = 1;
            for (;;) {
                if (parse_params(&argument, &g_params)) {
                    if (argument[0] == ',') {
                        argument++;
                        continue;
                    } else
            ...
            for (;;) {
                ...
                varInds_t v;
                for (v = 0; v < NUM_PARAMS; v++) {
                    if (longCommandWArg(&argument, g_shortParamNames[v]) ||
                        longCommandWArg(&argument, g_paramNames[v])) {
                        g_silenceParams[v] = 0;
                        found = 1;
                    }
                }
                if (longCommandWArg(&argument, "compressionParameters") ||
                    longCommandWArg(&argument, "cParams")) {
                    for (v = 0; v <= strt_ind; v++) {
                        g_silenceParams[v] = 0;
                    }
                    found = 1;
            ...
            while (argument[0] != 0) {
                ...
                case 'S':
                    ...
                    for (;;) {
                        ...
                        case 'w':
                            argument++;
                            g_params.vals[wlog_ind] =
                                readU32FromChar(&argument);
                            continue;
                        case 'c':
                            argument++;
                            g_params.vals[clog_ind] =
                                readU32FromChar(&argument);
                            continue;
                        case 'h':
                            argument++;
                            g_params.vals[hlog_ind] =
                                readU32FromChar(&argument);
                            continue;
                        case 's':
                            argument++;
                            g_params.vals[slog_ind] =
                                readU32FromChar(&argument);
                            continue;
                        ...
                        case 't': /* target length */
                            argument++;
                            g_params.vals[tlen_ind] =
                                readU32FromChar(&argument);
                            continue;
                        case 'S': /* strategy */
                            argument++;
                            g_params.vals[strt_ind] =
                                readU32FromChar(&argument);
                            continue;
                        case 'f': /* forceAttachDict */
                            argument++;
                            g_params.vals[fadt_ind] =
                                readU32FromChar(&argument);
                            continue;
                        case 'L': {
                            argument++;
    ...
    if (filenamesStart == 0) {
        ...
        if (seperateFiles) {
            for (i = 0; i < argc - filenamesStart; i++) {
                if (g_optimizer) {
                    result = optimizeForSize(
                        argv + filenamesStart + i, 1, dictFileName, target,
                        paramTarget, cLevelOpt, cLevelRun, memoTableLog);
                    if (result) {
                        DISPLAY("Error on File %d", i);
                        return result;
                    }
                } else {
                    result = benchFiles(argv + filenamesStart + i, 1,
                                        dictFileName, cLevelRun);
                    if (result) {
                        DISPLAY("Error on File %d", i);
                        return result;
                    }
            ...
            if (g_optimizer) {
                result = optimizeForSize(
                    argv + filenamesStart, argc - filenamesStart, dictFileName,
                    target, paramTarget, cLevelOpt, cLevelRun, memoTableLog);
            } else {
                result =
                    benchFiles(argv + filenamesStart, argc - filenamesStart,
                               dictFileName, cLevelRun);
            }
        }
    }
