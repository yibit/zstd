...
static int
testOrder(size_t numThreads, size_t queueSize)
{
    struct data data;
...
/* Tests for deadlock */
static int
testWait(size_t numThreads, size_t queueSize)
{
    struct data data;
    POOL_ctx *ctx = POOL_create(numThreads, queueSize);
    ASSERT_TRUE(ctx);
...
static int
testThreadReduction_internal(POOL_ctx *ctx, poolTest_t test)
{
...
static int
testAbruptEnding_internal(abruptEndCanary_t test)
{
    int const nbWaits = 16;
    ...
    ASSERT_EQ(POOL_resize(ctx, 1 /*numThreads*/),
              0); /* downsize numThreads, to try to break end condition */
