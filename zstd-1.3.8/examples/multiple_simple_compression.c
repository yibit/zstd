 ...
 */
static resources
createResources_orDie(int argc, const char **argv, char **ofn,
                      int *ofnBufferLen)
{
...
static void
freeResources(resources ress, char *outFilename)
{
    free(ress.fBuffer);
    free(ress.cBuffer);
...
static void
compressFile_orDie(resources ress, const char *fname, const char *oname)
{
    size_t fSize = loadFile_orDie(fname, ress.fBuffer, ress.fBufferSize);

    size_t const cSize = ZSTD_compressCCtx(
        ress.cctx, ress.cBuffer, ress.cBufferSize, ress.fBuffer, fSize, 1);
...
main(int argc, const char **argv)
{
    ...
    resources const ress =
        createResources_orDie(argc, argv, &outFilename, &outFilenameBufferLen);
