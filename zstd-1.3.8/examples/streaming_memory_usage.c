...
main(int argc, char const *argv[])
{
    ...
    for (compressionLevel = 1; compressionLevel <= MAX_TESTED_LEVEL;
         compressionLevel++) {
#define INPUT_SIZE 5
#define COMPRESSED_SIZE 128
        ...
        /* forces compressor to use maximum memory size for given compression
         * level,
        ...
        if (wLog) { /* special mode : specific wLog */
            ...
            params = ZSTD_getParams(1 /*compressionLevel*/,
                                    ...
                                    0 /*no dictionary*/);
            size_t const error = ZSTD_initCStream_advanced(
                cstream, NULL, 0, params, ZSTD_CONTENTSIZE_UNKNOWN);
        ...
        size_t compressedSize;
        {
            ...
            ZSTD_outBuffer outBuff = {compressedData, sizeof(compressedData),
                                      0};
            size_t const cError =
                ZSTD_compressStream(cstream, &outBuff, &inBuff);
        ...
        /* forces decompressor to use maximum memory size, as decompressed size
         * is not known */
        {
            ZSTD_inBuffer inBuff = {compressedData, compressedSize, 0};
            ZSTD_outBuffer outBuff = {decompressedData,
                                      sizeof(decompressedData), 0};
            size_t const dResult =
                ZSTD_decompressStream(dstream, &outBuff, &inBuff);
            ...
            if (dResult != 0) {
                printf(
                    "ZSTD_decompressStream error : unfinished decompression "
                    "\n");
                return 1;
            }
            if (outBuff.pos != sizeof(dataToCompress)) {
                printf(
                    "ZSTD_decompressStream error : incorrect decompression \n");
                return 1;
            }
        }

        size_t const cstreamSize = ZSTD_sizeof_CStream(cstream);
        size_t const cstreamEstimatedSize =
            wLog ? ZSTD_estimateCStreamSize_usingCParams(params.cParams)
                 : ZSTD_estimateCStreamSize(compressionLevel);
        size_t const dstreamSize = ZSTD_sizeof_DStream(dstream);

        printf(
            "Level %2i : Compression Mem = %5u KB (estimated : %5u KB) ; "
            "Decompression Mem = %4u KB \n",
            compressionLevel, (unsigned)(cstreamSize >> 10),
            (unsigned)(cstreamEstimatedSize >> 10),
            (unsigned)(dstreamSize >> 10));
