...
static void
decompressFile_orDie(const char *fname)
{
    ...
    /* In more complex scenarios, a file may consist of multiple appended frames
    * (ex : pzstd).
    ...
    *  It is compatible with other provided streaming examples */
    ...
    while ((read = fread_orDie(buffIn, toRead, fin))) {
        ...
        while (input.pos < input.size) {
            ...
            toRead = ZSTD_decompressStream(
                dstream, &output,
                &input); /* toRead : size of next compressed block */
