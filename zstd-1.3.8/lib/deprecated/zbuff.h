...
size_t
    ZBUFF_compressContinue(ZBUFF_CCtx *cctx, void *dst, size_t *dstCapacityPtr,
...
* caller to present again remaining data.
...
* call (it's just a hint, to improve latency)
*            or an error code, which can be tested using ZBUFF_isError().
*
*  At any moment, it's possible to flush whatever data remains within buffer,
* using ZBUFF_compressFlush().
*  The nb of bytes written into `dst` will be reported into *dstCapacityPtr.
*  Note that the function cannot output more than *dstCapacityPtr,
*  therefore, some content might still be left into internal buffer if
* *dstCapacityPtr is too small.
...
* internal buffer content if *dstCapacityPtr is too small.
...
*  input : ZBUFF_recommendedCInSize==128 KB block size is the internal unit, use
* this value to reduce intermediate stages (better latency)
*  output : ZBUFF_recommendedCOutSize==ZSTD_compressBound(128 KB) + 3 + 3 :
* ensures it's always possible to write/flush/end a full block. Skip some
* buffering.
*  By using both, it ensures that input will be entirely consumed, and output
* will always contain the result, reducing intermediate buffering.
* **************************************************/
...
* flush,
*            >1 when more data is expected, with value being a suggested next
* input size (it's just a hint, which helps latency),
*            or an error code, which can be tested using ZBUFF_isError().
*
*  Hint : recommended buffer sizes (not compulsory) : ZBUFF_recommendedDInSize()
* and ZBUFF_recommendedDOutSize()
*  output : ZBUFF_recommendedDOutSize== 128 KB block size is the internal unit,
* it ensures it's always possible to write a full block when decoded.
*  input  : ZBUFF_recommendedDInSize == 128KB + 3;
*           just follow indications from ZBUFF_decompressContinue() to minimize
