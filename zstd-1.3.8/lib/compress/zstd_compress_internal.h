...
#define ZSTD_DUBT_UNSORTED_MARK                                               \
    1 /* For btlazy2 strategy, index 1 now means "unsorted".                  \
         It could be confused for a real successor at index "1", if sorted as \
         larger than its predecessor.                                         \
         It's not a big deal though : candidate will just be sorted again.    \
         Additionnally, candidate position 1 will be lost.                    \
         But candidate 1 cannot hide a large tree of candidates, so it's a    \
         minimal loss.                                                        \
         The benefit is that ZSTD_DUBT_UNSORTED_MARK cannot be misdhandled    \
         after table re-use with a different strategy                         \
         Constant required by ZSTD_compressBlock_btlazy2() and                \
 ...
 * code) into seqStore_t.
 *  `offsetCode` : distance to match + 3 (values 1-3 are repCodes).
 *  `mlBase` : matchLength - MINMATCH
*/
MEM_STATIC void
ZSTD_storeSeq(seqStore_t *seqStorePtr, size_t litLength, const void *literals,
              U32 offsetCode, size_t mlBase)
{
                                       ...
                                       within a single segment */
    {
        ...
        DEBUGLOG(6, "Cpos%7u :%3u literals, match%4u bytes at offCode%7u", pos,
                 (U32)litLength, (U32)mlBase + MINMATCH, (U32)offsetCode);
    }
#endif
...
MEM_STATIC size_t
ZSTD_count(const BYTE *pIn, const BYTE *pMatch, const BYTE *const pInLimit)
{
 ...
 */
MEM_STATIC size_t
ZSTD_count_2segments(const BYTE *ip, const BYTE *match, const BYTE *iEnd,
                     const BYTE *mEnd, const BYTE *iStart)
{
...
static U32
ZSTD_hash3(U32 u, U32 h)
{
    return ((u << (32 - 24)) * prime3bytes) >> (32 - h);
}
MEM_STATIC size_t
ZSTD_hash3Ptr(const void *ptr, U32 h)
{
...
static U32
ZSTD_hash4(U32 u, U32 h)
{
    return (u * prime4bytes) >> (32 - h);
}
static size_t
ZSTD_hash4Ptr(const void *ptr, U32 h)
{
...
static size_t
ZSTD_hash5(U64 u, U32 h)
{
    return (size_t)(((u << (64 - 40)) * prime5bytes) >> (64 - h));
}
static size_t
ZSTD_hash5Ptr(const void *p, U32 h)
{
...
static size_t
ZSTD_hash7(U64 u, U32 h)
{
    return (size_t)(((u << (64 - 56)) * prime7bytes) >> (64 - h));
}
static size_t
ZSTD_hash7Ptr(const void *p, U32 h)
{
 ...
 */
static U64
ZSTD_ipow(U64 base, U64 exponent)
{
    U64 power = 1;
    while (exponent) {
        if (exponent & 1)
 ...
 */
static U64
ZSTD_rollingHash_append(U64 hash, void const *buf, size_t size)
{
 ...
 */
MEM_STATIC U64
ZSTD_rollingHash_compute(void const *buf, size_t size)
{
 ...
 */
MEM_STATIC U64
ZSTD_rollingHash_rotate(U64 hash, BYTE toRemove, BYTE toAdd, U64 primePower)
{
 ...
 */
MEM_STATIC void
ZSTD_window_clear(ZSTD_window_t *window)
{
 ...
 */
MEM_STATIC U32
ZSTD_window_hasExtDict(ZSTD_window_t const window)
{
...
ZSTD_matchState_dictMode(const ZSTD_matchState_t *ms)
{
    return ZSTD_window_hasExtDict(ms->window)
               ? ZSTD_extDict
               : ms->dictMatchState != NULL ? ZSTD_dictMatchState : ZSTD_noDict;
}
 ...
 */
MEM_STATIC U32
ZSTD_window_needOverflowCorrection(ZSTD_window_t const window,
                                   void const *srcEnd)
{
 ...
 */
MEM_STATIC U32
ZSTD_window_correctOverflow(ZSTD_window_t *window, U32 cycleLog, U32 maxDist,
                            void const *src)
{
 ...
 */
MEM_STATIC void
ZSTD_window_enforceMaxDist(ZSTD_window_t *window, void const *srcEnd,
 ...
 */
MEM_STATIC U32
ZSTD_window_update(ZSTD_window_t *window, void const *src, size_t srcSize)
{
...
MEM_STATIC void
ZSTD_debugTable(const U32 *table, U32 max)
{
    unsigned u, sum;
 ...
 */
size_t ZSTD_compressStream_generic(ZSTD_CStream *zcs, ZSTD_outBuffer *output,
                                   ZSTD_inBuffer *input,
