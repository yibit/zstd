...
void
ZSTD_fillHashTable(ZSTD_matchState_t *ms, void const *end,
...
size_t
ZSTD_compressBlock_fast_generic(ZSTD_matchState_t *ms, seqStore_t *seqStore,
                                ...
                                ZSTD_dictMode_e const dictMode)
{
    ...
    const U32 dictHLog =
        dictMode == ZSTD_dictMatchState ? dictCParams->hashLog : hlog;
    ...
    while (ip < ilimit) { /* < instead of <=, because repcode check at (ip+1) */
        ...
        const BYTE *repMatch =
            (dictMode == ZSTD_dictMatchState && repIndex < prefixStartIndex)
        ...
        if ((dictMode == ZSTD_dictMatchState) &&
            ((U32)((prefixStartIndex - 1) - repIndex) >=
             3) /* intentional underflow : ensure repIndex isn't overlapping
                   dict + prefix */
            ...
            if (dictMode == ZSTD_dictMatchState) {
                ...
                if (dictMatchIndex <= dictStartIndex ||
                    ...
                    /* found a dict match */
                    U32 const offset =
                        (U32)(current - dictMatchIndex - dictIndexDelta);
                    mLength = ZSTD_count_2segments(ip + 4, dictMatch + 4, iend,
                                                   dictEnd, prefixStart) +
                              4;
...
size_t
ZSTD_compressBlock_fast(ZSTD_matchState_t *ms, seqStore_t *seqStore,
...
size_t
ZSTD_compressBlock_fast_dictMatchState(ZSTD_matchState_t *ms,
                                       seqStore_t *seqStore,
                                       U32 rep[ZSTD_REP_NUM], void const *src,
                                       size_t srcSize)
{
...
size_t
ZSTD_compressBlock_fast_extDict(ZSTD_matchState_t *ms, seqStore_t *seqStore,
                                U32 rep[ZSTD_REP_NUM], void const *src,
                                size_t srcSize)
{
    ZSTD_compressionParameters const *cParams = &ms->cParams;
