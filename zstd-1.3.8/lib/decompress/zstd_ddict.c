...
const void *
ZSTD_DDict_dictContent(const ZSTD_DDict *ddict)
{
    assert(ddict != NULL);
...
size_t
ZSTD_DDict_dictSize(const ZSTD_DDict *ddict)
{
    assert(ddict != NULL);
...
void
ZSTD_copyDDictParameters(ZSTD_DCtx *dctx, const ZSTD_DDict *ddict)
{
                           ...
                           ZSTD_dictContentType_e dictContentType)
{
    ...
    }
    {
        ...
        if (magic != ZSTD_MAGIC_DICTIONARY) {
            if (dictContentType == ZSTD_dct_fullDict)
                return ERROR(dictionary_corrupted); /* only accept specified
                                                       dictionaries */
            return 0;                               /* pure content mode */
        }
    }
...
ZSTD_DDict *
ZSTD_createDDict_advanced(const void *dict, size_t dictSize,
                          ZSTD_dictLoadMethod_e dictLoadMethod,
                          ZSTD_dictContentType_e dictContentType,
                          ZSTD_customMem customMem)
{
...

    {
        ZSTD_DDict *const ddict =
            (ZSTD_DDict *)ZSTD_malloc(sizeof(ZSTD_DDict), customMem);
        if (ddict == NULL)
            return NULL;
...
ZSTD_DDict *
ZSTD_createDDict(const void *dict, size_t dictSize)
{
 ...
 * decompression.
 *  Warning : dictBuffer must outlive DDict (DDict must be freed before
 * dictBuffer) */
ZSTD_DDict *
...
size_t
ZSTD_sizeof_DDict(const ZSTD_DDict *ddict)
{
    if (ddict == NULL)
 ...
 * dictionaries. */
unsigned
ZSTD_getDictID_fromDDict(const ZSTD_DDict *ddict)
{
    if (ddict == NULL)
        return 0;
