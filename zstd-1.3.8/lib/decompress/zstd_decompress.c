 ...
 * Note that functions with explicit context such as ZSTD_decompressDCtx() are
 * unaffected.
 */
...
***************************************************************/
size_t
ZSTD_sizeof_DCtx(const ZSTD_DCtx *dctx)
{
    if (dctx == NULL)
...
{
    dctx->format = ZSTD_f_zstd1; /* ZSTD_decompressBegin() invokes
                                    ZSTD_startingInputLength() with argument
                                    dctx->format */
    dctx->staticSize = 0;
...
ZSTD_createDCtx_advanced(ZSTD_customMem customMem)
{
...

    {
        ZSTD_DCtx *const dctx =
            (ZSTD_DCtx *)ZSTD_malloc(sizeof(*dctx), customMem);
        if (!dctx)
            return NULL;
...
/* no longer useful */
void
ZSTD_copyDCtx(ZSTD_DCtx *dstDCtx, const ZSTD_DCtx *srcDCtx)
{
    size_t const toCopy =
        (size_t)((char *)(&dstDCtx->inBuff) - (char *)dstDCtx);
...
unsigned
ZSTD_isFrame(const void *buffer, size_t size)
{
...
static size_t
ZSTD_frameHeaderSize_internal(const void *src, size_t srcSize,
...
size_t
ZSTD_frameHeaderSize(const void *src, size_t srcSize)
{
...
size_t
ZSTD_getFrameHeader_advanced(ZSTD_frameHeader *zfhPtr, const void *src,
                             size_t srcSize, ZSTD_format_e format)
{
    ...
    memset(zfhPtr, 0,
           sizeof(*zfhPtr)); /* not strictly necessary, but static analyzer do
                                not understand that zfhPtr is only going to be
                                read only if return value is zero, since they
                                are 2 different signals */
    if (srcSize < minInputSize)
        return minInputSize;
    if (src == NULL)
...

    {
        ...
        if ((fhdByte & 0x08) != 0)
            return ERROR(
                frameParameter_unsupported); /* reserved bits, must be zero */
...
unsigned long long
ZSTD_getFrameContentSize(const void *src, size_t srcSize)
{
#if defined(ZSTD_LEGACY_SUPPORT) && (ZSTD_LEGACY_SUPPORT >= 1)
...
static size_t
readSkippableFrameSize(void const *src, size_t srcSize)
{
...
unsigned long long
ZSTD_findDecompressedSize(const void *src, size_t srcSize)
{
...
unsigned long long
ZSTD_getDecompressedSize(const void *src, size_t srcSize)
{
 ...
 * ZSTD_isError() */
static size_t
ZSTD_decodeFrameHeader(ZSTD_DCtx *dctx, const void *src, size_t headerSize)
{
    size_t const result = ZSTD_getFrameHeader_advanced(
        &(dctx->fParams), src, headerSize, dctx->format);
...
/** ZSTD_insertBlock() :
   ...
   */
size_t
ZSTD_insertBlock(ZSTD_DCtx *dctx, const void *blockStart, size_t blockSize)
{
...
static size_t
ZSTD_copyRawBlock(void *dst, size_t dstCapacity, const void *src,
                  size_t srcSize)
{
...
static size_t
ZSTD_setRleBlock(void *dst, size_t dstCapacity, BYTE b, size_t regenSize)
{
    if (dst == NULL) {
        if (regenSize == 0)
            return 0;
...
static size_t
ZSTD_decompressFrame(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
    ...
    while (1) {
        ...
        case bt_compressed:
            decodedSize = ZSTD_decompressBlock_internal(
                dctx, op, oend - op, ip, cBlockSize, /* frame */ 1);
            break;
        case bt_raw:
        ...
        case bt_rle:
            decodedSize =
                ZSTD_setRleBlock(op, oend - op, *ip, blockProperties.origSize);
            break;
        case bt_reserved:
        default:
...
static size_t
ZSTD_decompressMultiFrame(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
                          const void *src, size_t srcSize, const void *dict,
                          size_t dictSize, const ZSTD_DDict *ddict)
{
    ...
    while (srcSize >= ZSTD_FRAMEHEADERSIZE_PREFIX) {
        ...
        if (ZSTD_isLegacy(src, srcSize)) {
            ...
            size_t const frameSize =
                ZSTD_findFrameCompressedSizeLegacy(src, srcSize);
            ...
            decodedSize = ZSTD_decompressLegacy(dst, dstCapacity, src,
                                                frameSize, dict, dictSize);
            if (ZSTD_isError(decodedSize))
                return decodedSize;
...

        {
            ...
            if ((ZSTD_getErrorCode(res) == ZSTD_error_prefix_unknown) &&
                 ...
                 * which would be confusing, as the first header is actually
                 * correct.
                 * Note that one could be unlucky, it might be a corruption
                 * error instead,
...
size_t
ZSTD_decompress_usingDict(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
                          const void *src, size_t srcSize, const void *dict,
                          size_t dictSize)
{
...
size_t
ZSTD_decompressDCtx(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
                    const void *src, size_t srcSize)
{
    return ZSTD_decompress_usingDict(dctx, dst, dstCapacity, src, srcSize, NULL,
                                     0);
}
...
ZSTD_nextInputType(ZSTD_DCtx *dctx)
{
    switch (dctx->stage) {
 ...
 *  srcSize : must be the exact nb of bytes expected (see
 * ZSTD_nextSrcSizeToDecompress())
 *  @return : nb of bytes generated into `dst` (necessarily <= `dstCapacity)
 *            or an error code, which can be tested using ZSTD_isError() */
size_t
ZSTD_decompressContinue(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
                        const void *src, size_t srcSize)
{
        ...
        DEBUGLOG(5, "ZSTD_decompressContinue: case ZSTDds_decompressBlock");
        {
            ...
            case bt_compressed:
                ...
                rSize = ZSTD_decompressBlock_internal(
                    dctx, dst, dstCapacity, src, srcSize, /* frame */ 1);
                break;
            case bt_raw:
            ...
            if (dctx->stage == ZSTDds_decompressLastBlock) { /* end of frame */
                DEBUGLOG(
                    4, "ZSTD_decompressContinue: decoded size from frame : %u",
                ...
                if (dctx->fParams
                        .checksumFlag) { /* another round for frame checksum */
                    dctx->expected = 4;
                    dctx->stage = ZSTDds_checkChecksum;
                } else {
    ...
    case ZSTDds_checkChecksum:
        assert(srcSize == 4); /* guaranteed by dctx->expected */
        {
            ...
            DEBUGLOG(4,
                     "ZSTD_decompressContinue: checksum : calculated %08X :: "
                     "%08X read",
    ...
    case ZSTDds_decodeSkippableHeader:
        ...
        dctx->expected =
            MEM_readLE32(dctx->headerBuffer +
                         ZSTD_FRAMEIDSIZE); /* note : dctx->expected can grow
                                               seriously large, beyond local
                                               buffer size */
    ...
    default:
        assert(0); /* impossible */
        return ERROR(
            GENERIC); /* some compiler require default to do something */
    }
}

static size_t
ZSTD_refDictContent(ZSTD_DCtx *dctx, const void *dict, size_t dictSize)
{
    dctx->dictEnd = dctx->previousDstEnd;
    dctx->virtualStart =
...
size_t
ZSTD_loadDEntropy(ZSTD_entropyDTables_t *entropy, const void *const dict,
                  size_t const dictSize)
{
    ...
    assert(MEM_readLE32(dict) ==
           ZSTD_MAGIC_DICTIONARY); /* dict must be valid */
    ...
    {
        void *const workspace = &entropy->LLTable; /* use fse tables as
                                                      temporary workspace;
                                                      implies fse tables are
                                                      grouped together */
...

    {
        ...
        if (matchlengthLog > MLFSELog)
            return ERROR(dictionary_corrupted);
        ZSTD_buildFSETable(entropy->MLTable, matchlengthNCount,
                           matchlengthMaxValue, ML_base, ML_bits,
                           matchlengthLog);
...
static size_t
ZSTD_decompress_insertDictionary(ZSTD_DCtx *dctx, const void *dict,
                                 size_t dictSize)
{
    if (dictSize < 8)
...
size_t
ZSTD_decompressBegin_usingDict(ZSTD_DCtx *dctx, const void *dict,
                               size_t dictSize)
{
...
unsigned
ZSTD_getDictID_fromDict(const void *dict, size_t dictSize)
{
    if (dictSize < 8)
        return 0;
...
unsigned
ZSTD_getDictID_fromFrame(const void *src, size_t srcSize)
{
...
size_t
ZSTD_decompress_usingDDict(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
                           const void *src, size_t srcSize,
...
size_t
ZSTD_DStreamInSize(void)
{
    return ZSTD_BLOCKSIZE_MAX + ZSTD_blockHeaderSize;
}
size_t
ZSTD_DStreamOutSize(void)
{
    return ZSTD_BLOCKSIZE_MAX;
}

size_t
ZSTD_DCtx_loadDictionary_advanced(ZSTD_DCtx *dctx, const void *dict,
                                  size_t dictSize,
...
size_t
ZSTD_DCtx_loadDictionary_byReference(ZSTD_DCtx *dctx, const void *dict,
                                     size_t dictSize)
{
...
size_t
ZSTD_DCtx_loadDictionary(ZSTD_DCtx *dctx, const void *dict, size_t dictSize)
{
    return ZSTD_DCtx_loadDictionary_advanced(dctx, dict, dictSize,
...
size_t
ZSTD_DCtx_refPrefix_advanced(ZSTD_DCtx *dctx, const void *prefix,
                             size_t prefixSize,
                             ZSTD_dictContentType_e dictContentType)
{
...
size_t
ZSTD_DCtx_refPrefix(ZSTD_DCtx *dctx, const void *prefix, size_t prefixSize)
{
...
size_t
ZSTD_initDStream_usingDict(ZSTD_DStream *zds, const void *dict, size_t dictSize)
{
 ...
 * this function cannot fail */
size_t
ZSTD_initDStream_usingDDict(ZSTD_DStream *dctx, const ZSTD_DDict *ddict)
{
...
size_t
ZSTD_DCtx_refDDict(ZSTD_DCtx *dctx, const ZSTD_DDict *ddict)
{
    if (dctx->streamStage != zdss_init)
        return ERROR(stage_wrong);
    dctx->ddict = ddict;
...
size_t
ZSTD_DCtx_setMaxWindowSize(ZSTD_DCtx *dctx, size_t maxWindowSize)
{
 ...
 * 0 otherwise */
static int
ZSTD_dParam_withinBounds(ZSTD_dParameter dParam, int value)
{
...
size_t
ZSTD_DCtx_setParameter(ZSTD_DCtx *dctx, ZSTD_dParameter dParam, int value)
{
    if (dctx->streamStage != zdss_init)
        return ERROR(stage_wrong);
    switch (dParam) {
...
size_t
ZSTD_sizeof_DStream(const ZSTD_DStream *dctx)
{
    return ZSTD_sizeof_DCtx(dctx);
}

size_t
ZSTD_decodingBufferSize_min(unsigned long long windowSize,
...
size_t
ZSTD_estimateDStreamSize(size_t windowSize)
{
    size_t const blockSize = MIN(windowSize, ZSTD_BLOCKSIZE_MAX);
    size_t const inBuffSize = blockSize; /* no block can be larger */
    size_t const outBuffSize =
        ZSTD_decodingBufferSize_min(windowSize, ZSTD_CONTENTSIZE_UNKNOWN);
    return ZSTD_estimateDCtxSize() + inBuffSize + outBuffSize;
}

size_t
ZSTD_estimateDStreamSize_fromFrame(const void *src, size_t srcSize)
{
    U32 const windowSizeMax =
        1U << ZSTD_WINDOWLOG_MAX; /* note : should be user-selectable, but
                                     requires an additional parameter (or a
                                     dctx) */
    ZSTD_frameHeader zfh;
...
size_t
ZSTD_decompressStream(ZSTD_DStream *zds, ZSTD_outBuffer *output,
                      ZSTD_inBuffer *input)
{
    ...
    if (input->pos > input->size) { /* forbidden */
        DEBUGLOG(5, "in: pos: %u   vs size: %u", (U32)input->pos,
                 (U32)input->size);
        return ERROR(srcSize_wrong);
    }
    if (output->pos > output->size) { /* forbidden */
        DEBUGLOG(5, "out: pos: %u   vs size: %u", (U32)output->pos,
                 (U32)output->size);
        return ERROR(dstSize_tooSmall);
    }
    ...
    while (someMoreWork) {
        ...
        case zdss_init:
            ...
            ZSTD_resetDStream(
                zds); /* transparent reset on starting decoding a new frame */
        /* fall-through */
        ...
        case zdss_loadHeader:
...
#endif
            {
                size_t const hSize = ZSTD_getFrameHeader_advanced(
                    &zds->fParams, zds->headerBuffer, zds->lhSize, zds->format);
                DEBUGLOG(5, "header size : %u", (U32)hSize);
                if (ZSTD_isError(hSize)) {
#if defined(ZSTD_LEGACY_SUPPORT) && (ZSTD_LEGACY_SUPPORT >= 1)
                    U32 const legacyVersion =
                        ZSTD_isLegacy(istart, iend - istart);
                    if (legacyVersion) {
                        const void *const dict =
                            zds->ddict ? ZSTD_DDict_dictContent(zds->ddict)
                                       : NULL;
                        size_t const dictSize =
                            zds->ddict ? ZSTD_DDict_dictSize(zds->ddict) : 0;
                        DEBUGLOG(5,
                                 "ZSTD_decompressStream: detected legacy "
                                 "version v0.%u",
                                 legacyVersion);
                        /* legacy support is incompatible with static dctx */
                        if (zds->staticSize)
                        ...
                        CHECK_F(ZSTD_initLegacyStream(
                            &zds->legacyContext, zds->previousLegacyVersion,
                            legacyVersion, dict, dictSize));
                        zds->legacyVersion = zds->previousLegacyVersion =
                            legacyVersion;
                        {
                            size_t const hint = ZSTD_decompressLegacyStream(
                                zds->legacyContext, legacyVersion, output,
                                input);
                            if (hint == 0)
                                zds->streamStage =
                                    zdss_init; /* or stay in stage
                                                  zdss_loadHeader */
                            return hint;
                        }
                    }
                ...
                if (hSize != 0) { /* need more input */
                    size_t const toLoad =
                        hSize -
                        zds->lhSize; /* if hSize!=0, hSize > zds->lhSize */
                    size_t const remainingInput = (size_t)(iend - ip);
                    assert(iend >= ip);
                    if (toLoad > remainingInput) { /* not enough input to load
                                                      full header */
                        if (remainingInput > 0) {
                        ...
                        return (MAX(ZSTD_FRAMEHEADERSIZE_MIN, hSize) -
                                zds->lhSize) +
                               ZSTD_blockHeaderSize; /* remaining header bytes +
                                                        next block header */
                    }
                    assert(ip != NULL);
            ...
            if (zds->fParams.frameContentSize &&
                ...
                if (cSize <= (size_t)(iend - istart)) {
                    ...
                    size_t const decompressedSize = ZSTD_decompress_usingDDict(
                        zds, op, oend - op, istart, cSize, zds->ddict);
            ...
            if ((MEM_readLE32(zds->headerBuffer) & ZSTD_MAGIC_SKIPPABLE_MASK) ==
                ZSTD_MAGIC_SKIPPABLE_START) { /* skippable frame */
                zds->expected =
                    MEM_readLE32(zds->headerBuffer + ZSTD_FRAMEIDSIZE);
                zds->stage = ZSTDds_skipFrame;
            } else {
                CHECK_F(ZSTD_decodeFrameHeader(zds, zds->headerBuffer,
                                               zds->lhSize));
            ...
            /* Adapt buffer sizes to frame header instructions */
            {
                ...
                if ((zds->inBuffSize < neededInBuffSize) ||
                    ...
                    if (zds->staticSize) { /* static DCtx */
                        ...
                        assert(zds->staticSize >=
                               sizeof(ZSTD_DCtx)); /* controlled at init */
                        ...
                        zds->outBuffSize = 0;
                        zds->inBuff =
                            (char *)ZSTD_malloc(bufferSize, zds->customMem);
                        if (zds->inBuff == NULL)
                            return ERROR(memory_allocation);
                    }
        ...
        case zdss_load: {
            ...
            if (isSkipFrame) {
                ...
                loadedSize = ZSTD_limitCopy(zds->inBuff + zds->inPos, toLoad,
                                            ip, iend - ip);
            }
            ip += loadedSize;
            ...
            /* decode loaded input */
            {
                size_t const decodedSize =
                    ZSTD_decompressContinue(zds, zds->outBuff + zds->outStart,
        ...
        case zdss_flush: {
            ...
            size_t const flushedSize = ZSTD_limitCopy(
                op, oend - op, zds->outBuff + zds->outStart, toFlushSize);
            op += flushedSize;
            ...
            if (flushedSize == toFlushSize) { /* flush completed */
                ...
                if ((zds->outBuffSize < zds->fParams.frameContentSize) &&
                    (zds->outStart + zds->fParams.blockSizeMax >
                     zds->outBuffSize)) {
                    DEBUGLOG(5,
                             "restart filling outBuff from beginning (left:%i, "
                             "needed:%u)",
    ...
    }
    {
        ...
        if (!nextSrcSizeHint) {                 /* frame fully decoded */
            ...
            if (!zds
                     ->hostageByte) { /* output not fully flushed; keep last
                                         byte as hostage; will be released when
                                         all output is flushed */
                input
                    ->pos--; /* note : pos > 0, otherwise, impossible to finish
                                reading last block */
                zds->hostageByte = 1;
            }
            return 1;
...
size_t
ZSTD_decompressStream_simpleArgs(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
                                 size_t *dstPos, const void *src,
