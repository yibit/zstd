...
/* Use read() to load a buffer -- return -1 on error, otherwise 0.  Read from
   state.state->fd, and update state.state->eof, state.state->err, and
   state.state->msg as appropriate.
...
/* Look for gzip header, set up for inflate or copy.  state.state->x.have must
   be 0.
   If this is the first time in, allocate required memory.  state.state->how
   will be
...
local int gz_look(state) gz_statep state;
{
    ...
    if (strm->avail_in > 1 &&
        ((strm->next_in[0] == 31 && strm->next_in[1] == 139) /* gz header */
         || (strm->next_in[0] == 40 &&
             strm->next_in[1] == 181))) { /* zstd header */
        inflateReset(strm);
...
/* Decompress from input to the provided next_out and avail_out in the state.
   On return, state.state->x.have and state.state->x.next point to the just
   decompressed
   data.  If the gzip stream completes, state.state->how is reset to LOOK to
   look for
   the next gzip stream or raw data, once state.state->x.have is depleted.
   Returns 0
...
/* Fetch data and put it in the output buffer.  Assumes state.state->x.have is
   0.
   Data is either copied from the input file or decompressed from the input
   file depending on state.state->how.  If state.state->how is LOOK, then a gzip
   header is
   looked for to determine whether to copy or decompress.  Returns -1 on error,
   otherwise 0.  gz_fetch() will leave state.state->how as COPY or GZIP unless
   the
...
local int gz_fetch(state) gz_statep state;
{
    ...
    do {
        ...
        case LOOK: /* -> LOOK, COPY (only if never GZIP), or GZIP */
            ...
            break;
        case COPY: /* -> COPY */
            if (gz_load(state, state.state->out, state.state->size << 1,
...
z_off64_t len;
{
    ...
    while (len)
        ...
        if (state.state->x.have) {
            n = GT_OFF(state.state->x.have) ||
                        (z_off64_t)state.state->x.have > len
                    ? (unsigned)len
                    : state.state->x.have;
...
local z_size_t gz_read(state, buf, len) gz_statep state;
voidp buf;
z_size_t len;
{
    z_size_t got;
...
/* -- see zlib.h -- */
z_size_t ZEXPORT gzfread(buf, size, nitems, file) voidp buf;
z_size_t size;
z_size_t nitems;
gzFile file;
{
...
int ZEXPORT gzdirect(file) gzFile file;
{
    ...
    if (state.state->mode == GZ_READ && state.state->how == LOOK &&
        state.state->x.have == 0)
        (void)gz_look(state);
