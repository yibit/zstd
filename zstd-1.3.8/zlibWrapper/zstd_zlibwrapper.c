...
#define ZWRAP_DEFAULT_CLEVEL                                                  \
    3 /* Z_DEFAULT_COMPRESSION is translated to ZWRAP_DEFAULT_CLEVEL for zstd \
         */
...
void
ZWRAP_useZSTDcompression(int turn_on)
{
    g_ZWRAP_useZSTDcompression = turn_on;
}

int
ZWRAP_isUsingZSTDcompression(void)
{
    return g_ZWRAP_useZSTDcompression;
}

static ZWRAP_decompress_type g_ZWRAPdecompressionType = ZWRAP_AUTO;

void
ZWRAP_setDecompressionType(ZWRAP_decompress_type type)
{
    g_ZWRAPdecompressionType = type;
};

ZWRAP_decompress_type
ZWRAP_getDecompressionType(void)
{
    return g_ZWRAPdecompressionType;
}
...
ZEXTERN const char *ZEXPORT z_zlibVersion
OF((void))
{
    return zlibVersion();
}
...
static void *
ZWRAP_allocFunction(void *opaque, size_t size)
{
...
static void
ZWRAP_freeFunction(void *opaque, void *address)
{
    z_streamp strm = (z_streamp)opaque;
...
typedef struct {
    ...
    unsigned long long
        totalInBytes; /* we need it as strm->total_in can be reset by user */
...
static ZWRAP_CCtx *
ZWRAP_createCCtx(z_streamp strm)
{
    ZWRAP_CCtx *zwc;
    ...
    if (strm->zalloc && strm->zfree) {
        ...
        {
            ZSTD_customMem const ZWRAP_customMem = {
                ZWRAP_allocFunction, ZWRAP_freeFunction, &zwc->allocFunc};
...
static int
ZWRAP_initializeCStream(ZWRAP_CCtx *zwc, const void *dict, size_t dictSize,
                        unsigned long long pledgedSrcSize)
{
    ...
    if (!pledgedSrcSize)
        pledgedSrcSize = zwc->pledgedSrcSize;
    {
        ZSTD_parameters const params =
            ZSTD_getParams(zwc->compressionLevel, pledgedSrcSize, dictSize);
        size_t initErr;
        LOG_WRAPPERC(
            "pledgedSrcSize=%d windowLog=%d chainLog=%d hashLog=%d "
            "searchLog=%d minMatch=%d strategy=%d\n",
            (int)pledgedSrcSize, params.cParams.windowLog,
            params.cParams.chainLog, params.cParams.hashLog,
            params.cParams.searchLog, params.cParams.minMatch,
            params.cParams.strategy);
        initErr = ZSTD_initCStream_advanced(zwc->zbc, dict, dictSize, params,
                                            pledgedSrcSize);
...
static int
ZWRAPC_finishWithError(ZWRAP_CCtx *zwc, z_streamp strm, int error)
{
...
static int
ZWRAPC_finishWithErrorMsg(z_streamp strm, char *message)
{
...
int
ZWRAP_setPledgedSrcSize(z_streamp strm, unsigned long long pledgedSrcSize)
{
    ZWRAP_CCtx *zwc = (ZWRAP_CCtx *)strm->state;
    if (zwc == NULL)
        return Z_STREAM_ERROR;
...
ZEXTERN int ZEXPORT z_deflateInit_
OF((z_streamp strm, int level, const char *version, int stream_size))
{
    ...
    strm->state =
        (struct internal_state *)zwc; /* use state which in not used by user */
    strm->total_in = 0;
    strm->total_out = 0;
    strm->adler = 0;
    return Z_OK;
...
ZEXTERN int ZEXPORT z_deflateInit2_
...
int
ZWRAP_deflateReset_keepDict(z_streamp strm)
{
...
ZEXTERN int ZEXPORT z_deflateReset
OF((z_streamp strm))
{
...
ZEXTERN int ZEXPORT z_deflateSetDictionary
OF((z_streamp strm, const Bytef *dictionary, uInt dictLength))
{
...

    {
        ...
        {
            int res = ZWRAP_initializeCStream(zwc, dictionary, dictLength,
                                              ZSTD_CONTENTSIZE_UNKNOWN);
            if (res != Z_OK)
                return ZWRAPC_finishWithError(zwc, strm, res);
        }
...
OF((z_streamp strm, int flush))
{
    ...
    if (!g_ZWRAP_useZSTDcompression) {
        LOG_WRAPPERC(
            "- deflate1 flush=%d avail_in=%d avail_out=%d total_in=%d "
            "total_out=%d\n",
    ...
    if (zwc->zbc == NULL) {
        ...
        if (zwc->zbc == NULL)
            return ZWRAPC_finishWithError(zwc, strm, 0);
        {
            int const initErr = ZWRAP_initializeCStream(
                zwc, NULL, 0,
                (flush == Z_FINISH) ? strm->avail_in
                                    : ZSTD_CONTENTSIZE_UNKNOWN);
            if (initErr != Z_OK)
                return ZWRAPC_finishWithError(zwc, strm, initErr);
        }
        if (flush != Z_FINISH)
            zwc->comprState = ZWRAP_useReset;
    } else {
        if (zwc->totalInBytes == 0) {
            if (zwc->comprState == ZWRAP_useReset) {
                size_t const resetErr = ZSTD_resetCStream(
                    zwc->zbc,
                    (flush == Z_FINISH) ? strm->avail_in : zwc->pledgedSrcSize);
                ...
                int const res = ZWRAP_initializeCStream(
                    zwc, NULL, 0,
                    (flush == Z_FINISH) ? strm->avail_in
                                        : ZSTD_CONTENTSIZE_UNKNOWN);
                if (res != Z_OK)
    ...
    LOG_WRAPPERC(
        "- deflate2 flush=%d avail_in=%d avail_out=%d total_in=%d "
        "total_out=%d\n",
        (int)flush, (int)strm->avail_in, (int)strm->avail_out,
        (int)strm->total_in, (int)strm->total_out);
    if (strm->avail_in > 0) {
        ...
        {
            size_t const cErr =
                ZSTD_compressStream(zwc->zbc, &zwc->outBuffer, &zwc->inBuffer);
            LOG_WRAPPERC(
                "deflate ZSTD_compressStream srcSize=%d dstCapacity=%d\n",
                (int)zwc->inBuffer.size, (int)zwc->outBuffer.size);
            if (ZSTD_isError(cErr))
                return ZWRAPC_finishWithError(zwc, strm, 0);
        }
...
#endif
        || flush == Z_BLOCK)
        return ZWRAPC_finishWithErrorMsg(
            strm, "Z_FULL_FLUSH, Z_BLOCK and Z_TREES are not supported!");
    ...
    if (flush == Z_FINISH) {
        ...
        if (bytesLeft == 0) {
            zwc->streamEnd = 1;
            LOG_WRAPPERC(
                "Z_STREAM_END2 strm->total_in=%d strm->avail_out=%d "
                "strm->total_out=%d\n",
                (int)strm->total_in, (int)strm->avail_out,
                (int)strm->total_out);
            return Z_STREAM_END;
        }
    } else if (flush == Z_SYNC_FLUSH || flush == Z_PARTIAL_FLUSH) {
        size_t bytesLeft;
        zwc->outBuffer.dst = strm->next_out;
        zwc->outBuffer.size = strm->avail_out;
        zwc->outBuffer.pos = 0;
    ...
    LOG_WRAPPERC(
        "- deflate3 flush=%d avail_in=%d avail_out=%d total_in=%d "
        "total_out=%d\n",
        (int)flush, (int)strm->avail_in, (int)strm->avail_out,
        (int)strm->total_in, (int)strm->total_out);
    return Z_OK;
...
ZEXTERN int ZEXPORT z_deflateEnd
OF((z_streamp strm))
{
    if (!g_ZWRAP_useZSTDcompression) {
...
ZEXTERN uLong ZEXPORT z_deflateBound
...
static ZWRAP_DCtx *
ZWRAP_createDCtx(z_streamp strm)
{
    ZWRAP_DCtx *zwd;
    MEM_STATIC_ASSERT(
        sizeof(zwd->headerBuf) >=
        ZSTD_FRAMEHEADERSIZE_MIN); /* check static buffer size condition */
    ...
    if (strm->zalloc && strm->zfree) {
        ...
        {
            ZSTD_customMem const ZWRAP_customMem = {
                ZWRAP_allocFunction, ZWRAP_freeFunction, &zwd->allocFunc};
...
int
ZWRAP_isUsingZSTDdecompression(z_streamp strm)
{
    if (strm == NULL)
        return 0;
...
static int
ZWRAPD_finishWithError(ZWRAP_DCtx *zwd, z_streamp strm, int error)
{
...
static int
ZWRAPD_finishWithErrorMsg(z_streamp strm, char *message)
{
...
ZEXTERN int ZEXPORT z_inflateInit_
...
ZEXTERN int ZEXPORT z_inflateInit2_
...
int
ZWRAP_inflateReset_keepDict(z_streamp strm)
{
...
ZEXTERN int ZEXPORT z_inflateReset
OF((z_streamp strm))
{
...
#if ZLIB_VERNUM >= 0x1240
ZEXTERN int ZEXPORT z_inflateReset2
...

    {
            ...
            return Z_STREAM_ERROR;
        {
            size_t const initErr =
                ZSTD_initDStream_usingDict(zwd->zbd, dictionary, dictLength);
            if (ZSTD_isError(initErr))
                return ZWRAPD_finishWithError(zwd, strm, 0);
        }
        zwd->decompState = ZWRAP_useReset;
        ...
        if (zwd->totalInBytes == ZSTD_HEADERSIZE) {
            ...
            zwd->outBuffer.pos = 0;
            {
                size_t const errorCode = ZSTD_decompressStream(
                    zwd->zbd, &zwd->outBuffer, &zwd->inBuffer);
                LOG_WRAPPERD(
                    "inflateSetDictionary ZSTD_decompressStream errorCode=%d "
                    "srcSize=%d dstCapacity=%d\n",
                    (int)errorCode, (int)zwd->inBuffer.size,
                    (int)zwd->outBuffer.size);
                if (zwd->inBuffer.pos < zwd->outBuffer.size ||
                    ZSTD_isError(errorCode)) {
...
OF((z_streamp strm, int flush))
{
    ...
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved) {
        ...
        LOG_WRAPPERD(
            "- inflate2 flush=%d avail_in=%d avail_out=%d total_in=%d "
            "total_out=%d res=%d\n",
            (int)flush, (int)strm->avail_in, (int)strm->avail_out,
    ...
    LOG_WRAPPERD(
        "- inflate1 flush=%d avail_in=%d avail_out=%d total_in=%d "
        "total_out=%d\n",
        (int)flush, (int)strm->avail_in, (int)strm->avail_out,
        (int)strm->total_in, (int)strm->total_out);
    ...
    if (zwd == NULL)
        return Z_STREAM_ERROR;
    if (zwd->decompState == ZWRAP_streamEnd)
        return Z_STREAM_END;
        ...
        if (zwd->totalInBytes == 0 && strm->avail_in >= ZLIB_HEADERSIZE) {
            if (MEM_readLE32(strm->next_in) != ZSTD_MAGICNUMBER) {
                {
                    int const initErr =
                        (zwd->windowBits)
                            ? inflateInit2_(strm, zwd->windowBits, zwd->version,
                                            zwd->stream_size)
                            : inflateInit_(strm, zwd->version,
                                           zwd->stream_size);
                    LOG_WRAPPERD("ZLIB inflateInit errorCode=%d\n", initErr);
                    if (initErr != Z_OK)
                        return ZWRAPD_finishWithError(zwd, strm, initErr);
                }

                strm->reserved = ZWRAP_ZLIB_STREAM;
                {
                    size_t const freeErr = ZWRAP_freeDCtx(zwd);
                    if (ZSTD_isError(freeErr))
                        goto error;
                }

                {
                    int const result = (flush == Z_INFLATE_SYNC)
                                           ...
                                           : inflate(strm, flush);
                    LOG_WRAPPERD(
                        "- inflate3 flush=%d avail_in=%d avail_out=%d "
                        "total_in=%d total_out=%d res=%d\n",
                        (int)flush, (int)strm->avail_in, (int)strm->avail_out,
                        (int)strm->total_in, (int)strm->total_out, res);
                    return result;
                }
            }
        } else { /* ! (zwd->totalInBytes == 0 && strm->avail_in >=
                    ZLIB_HEADERSIZE) */
            size_t const srcSize =
                MIN(strm->avail_in, ZLIB_HEADERSIZE - zwd->totalInBytes);
            ...
            if (MEM_readLE32(zwd->headerBuf) != ZSTD_MAGICNUMBER) {
                ...
                strm->avail_out = 0;
                {
                    ...
                    LOG_WRAPPERD(
                        "ZLIB inflate errorCode=%d strm->avail_in=%d\n", dErr,
                        (int)strm->avail_in);
                    if (dErr != Z_OK)
...

                {
                    int const result = (flush == Z_INFLATE_SYNC)
                                           ...
                                           : inflate(strm, flush);
                    LOG_WRAPPERD(
                        "- inflate2 flush=%d avail_in=%d avail_out=%d "
                        "total_in=%d total_out=%d res=%d\n",
                        (int)flush, (int)strm->avail_in, (int)strm->avail_out,
                        (int)strm->total_in, (int)strm->total_out, res);
                    return result;
    ...
    if (flush == Z_INFLATE_SYNC) {
        strm->msg = "inflateSync is not supported!";
        goto error;
    }
    ...
    if (!zwd->zbd) {
        ...
        if (zwd->zbd == NULL) {
            LOG_WRAPPERD("ERROR: ZSTD_createDStream_advanced\n");
            goto error;
        }
        zwd->decompState = ZWRAP_useInit;
    }
    ...
    if (zwd->totalInBytes < ZSTD_HEADERSIZE) {
        if (zwd->totalInBytes == 0 && strm->avail_in >= ZSTD_HEADERSIZE) {
            ...
            size_t const srcSize =
                MIN(strm->avail_in, ZSTD_HEADERSIZE - zwd->totalInBytes);
            memcpy(zwd->headerBuf + zwd->totalInBytes, strm->next_in, srcSize);
            strm->total_in += srcSize;
            zwd->totalInBytes += srcSize;
            strm->next_in += srcSize;
            ...
            zwd->outBuffer.pos = 0;
            {
                size_t const dErr = ZSTD_decompressStream(
                    zwd->zbd, &zwd->outBuffer, &zwd->inBuffer);
                LOG_WRAPPERD(
                    "inflate ZSTD_decompressStream1 errorCode=%d srcSize=%d "
                    "dstCapacity=%d\n",
                    (int)dErr, (int)zwd->inBuffer.size,
                    (int)zwd->outBuffer.size);
            ...
            if (zwd->inBuffer.pos != zwd->inBuffer.size)
                goto error; /* not consumed */
        }
    ...
    {
        size_t const dErr =
            ZSTD_decompressStream(zwd->zbd, &zwd->outBuffer, &zwd->inBuffer);
        LOG_WRAPPERD(
            "inflate ZSTD_decompressStream2 errorCode=%d srcSize=%d "
            "dstCapacity=%d\n",
            (int)dErr, (int)strm->avail_in, (int)strm->avail_out);
        if (ZSTD_isError(dErr)) {
            zwd->errorCount++;
            LOG_WRAPPERD(
                "ERROR: ZSTD_decompressStream2 %s zwd->errorCount=%d\n",
        ...
        LOG_WRAPPERD(
            "inflate inBuffer.pos=%d inBuffer.size=%d outBuffer.pos=%d "
            "outBuffer.size=%d o\n",
            (int)zwd->inBuffer.pos, (int)zwd->inBuffer.size,
            (int)zwd->outBuffer.pos, (int)zwd->outBuffer.size);
        ...
        if (dErr == 0) {
            LOG_WRAPPERD(
                "inflate Z_STREAM_END1 avail_in=%d avail_out=%d total_in=%d "
                "total_out=%d\n",
                (int)strm->avail_in, (int)strm->avail_out, (int)strm->total_in,
                (int)strm->total_out);
    ...
    LOG_WRAPPERD(
        "- inflate2 flush=%d avail_in=%d avail_out=%d total_in=%d total_out=%d "
        "res=%d\n",
        (int)flush, (int)strm->avail_in, (int)strm->avail_out,
        (int)strm->total_in, (int)strm->total_out, Z_OK);
    return Z_OK;
...
ZEXTERN int ZEXPORT z_inflateEnd
OF((z_streamp strm))
{
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
...
ZEXTERN int ZEXPORT z_inflateSync
OF((z_streamp strm))
{
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved) {
...
ZEXTERN int ZEXPORT z_deflateTune
...
ZEXTERN int ZEXPORT z_deflatePending
...
ZEXTERN int ZEXPORT z_deflatePrime
...
ZEXTERN int ZEXPORT z_deflateSetHeader
OF((z_streamp strm, gz_headerp head))
{
    if (!g_ZWRAP_useZSTDcompression)
        return deflateSetHeader(strm, head);
    return ZWRAPC_finishWithErrorMsg(strm,
                                     "deflateSetHeader is not supported!");
}

/* Advanced decompression functions */
#if ZLIB_VERNUM >= 0x1280
ZEXTERN int ZEXPORT z_inflateGetDictionary
OF((z_streamp strm, Bytef *dictionary, uInt *dictLength))
{
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
        return inflateGetDictionary(strm, dictionary, dictLength);
    return ZWRAPD_finishWithErrorMsg(strm,
                                     "inflateGetDictionary is not supported!");
}
#endif

ZEXTERN int ZEXPORT z_inflateCopy
OF((z_streamp dest, z_streamp source))
{
...
#if ZLIB_VERNUM >= 0x1240
ZEXTERN long ZEXPORT z_inflateMark
OF((z_streamp strm))
{
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
...
ZEXTERN int ZEXPORT z_inflatePrime
OF((z_streamp strm, int bits, int value))
{
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
...
ZEXTERN int ZEXPORT z_inflateGetHeader
OF((z_streamp strm, gz_headerp head))
{
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
        return inflateGetHeader(strm, head);
    return ZWRAPD_finishWithErrorMsg(strm,
                                     "inflateGetHeader is not supported!");
}

ZEXTERN int ZEXPORT z_inflateBackInit_
...
ZEXTERN int ZEXPORT z_inflateBack
...
ZEXTERN int ZEXPORT z_inflateBackEnd
OF((z_streamp strm))
{
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
...
ZEXTERN uLong ZEXPORT z_zlibCompileFlags
OF((void))
{
...
ZEXTERN int ZEXPORT z_compress
OF((Bytef * dest, uLongf *destLen, const Bytef *source, uLong sourceLen))
{
...

    {
        ...
        size_t const cSize = ZSTD_compress(dest, dstCapacity, source, sourceLen,
...
ZEXTERN int ZEXPORT z_compress2
OF((Bytef * dest, uLongf *destLen, const Bytef *source, uLong sourceLen,
    int level))
{
    if (!g_ZWRAP_useZSTDcompression)
...

    {
        size_t dstCapacity = *destLen;
        size_t const cSize =
            ZSTD_compress(dest, dstCapacity, source, sourceLen, level);
        if (ZSTD_isError(cSize))
            return Z_STREAM_ERROR;
        *destLen = cSize;
    }
...
ZEXTERN uLong ZEXPORT z_compressBound
OF((uLong sourceLen))
{
    if (!g_ZWRAP_useZSTDcompression)
...
ZEXTERN int ZEXPORT z_uncompress
OF((Bytef * dest, uLongf *destLen, const Bytef *source, uLong sourceLen))
{
...

    {
        size_t dstCapacity = *destLen;
        size_t const dSize =
            ZSTD_decompress(dest, dstCapacity, source, sourceLen);
...
ZEXTERN uLong ZEXPORT z_adler32
...
ZEXTERN uLong ZEXPORT z_adler32_z
OF((uLong adler, const Bytef *buf, z_size_t len))
{
    return adler32_z(adler, buf, len);
}

ZEXTERN uLong ZEXPORT z_crc32_z
OF((uLong crc, const Bytef *buf, z_size_t len))
{
...
ZEXTERN const z_crc_t FAR *ZEXPORT z_get_crc_table
OF((void))
{
