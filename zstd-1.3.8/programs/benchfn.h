 ...
 * BMK_runOutcome_t must be considered "opaque" : never access its members
 * directly.
 ...
 * No initializer is provided, because it doesn't make sense to provide some
 * "default" :
 * all parameters should be specified by the caller */
typedef struct {
    BMK_benchFn_t
        benchFn; /* the function to benchmark, over the set of blocks */
    void *
        benchPayload;    /* pass custom parameters to benchFn  :
                          * (*benchFn)(srcBuffers[i], srcSizes[i], dstBuffers[i],
                          * dstCapacities[i], benchPayload) */
    BMK_initFn_t initFn; /* (*initFn)(initPayload) is run once per run, at the
                            beginning. */
    void *initPayload;   /* Both arguments can be NULL, in which case nothing is
                            run. */
    BMK_errorFn_t
        errorFn; /* errorFn will check each return value of benchFn over each
                  * block, to determine if it failed or not.
                  * errorFn can be NULL, in which case no check is performed.
                  * errorFn must return 0 when benchFn was successful, and >= 1
                  * if it detects an error.
                  * Execution is stopped as soon as an error is detected.
                  * the triggering return value can be retrieved using
    ...
    size_t blockCount; /* number of blocks to operate benchFn on.
                        ...
                        * srcBuffers, srcSizes, dstBuffers, dstCapacities,
                        * blockResults */
    const void *const
        *srcBuffers;        /* array of buffers to be operated on by benchFn */
    const size_t *srcSizes; /* array of the sizes of srcBuffers buffers */
    void *const
        *dstBuffers; /* array of buffers to be written into by benchFn */
    const size_t
        *dstCapacities;   /* array of the capacities of dstBuffers buffers */
    size_t *blockResults; /* Optional: store the return value of benchFn for
                             each block. Use NULL if this result is not
                             requested. */
} BMK_benchParams_t;
 ...
 *          Use BMK_isSuccessful_runOutcome() to check if benchmark was
 * successful.
 *          If yes, extract the result with BMK_extract_runTime(),
 *          it will contain :
 *              .sumOfReturn : the sum of all return values of benchFn through
 * all of blocks
 *              .nanoSecPerRun : time per run of benchFn + (time for initFn /
 * nbLoops)
 *          .sumOfReturn is generally intended for functions which return a # of
 * bytes written into dstBuffer,
 *              in which case, this value will be the total amount of bytes
 * written into dstBuffer.
 *
 * blockResults : when provided (!= NULL), and when benchmark is successful,
 *                params.blockResults contains all return values of `benchFn`
 * over all blocks.
 *                when provided (!= NULL), and when benchmark failed,
 *                params.blockResults contains return values of `benchFn` over
 * all blocks preceding and including the failed block.
 */
 ...
 * The failure was detected by `errorFn`, operating on return values of
 * `benchFn`.
 * Returns the faulty return value.
 * note : this function will abort() program execution if benchmark did not
 * failed.
 ...
 * Automatically determines `nbLoops` so that each result is regularly produced
 * at interval of about run_ms.
 * Note : minimum `nbLoops` is 1, therefore a run may last more than run_ms, and
 * possibly even more than total_ms.
 * Usage - initialize timedFnState, select benchmark duration (total_ms) and
 * each measurement duration (run_ms)
 *         call BMK_benchTimedFn() repetitively, each measurement is supposed to
 * last about run_ms
 *         Check if total time budget is spent or exceeded, using
...
void BMK_resetTimedFnState(BMK_timedFnState_t *timedFnState, unsigned total_ms,
                           unsigned run_ms);
