...
static void
BMK_initCCtx(ZSTD_CCtx *ctx, const void *dictBuffer, size_t dictBufferSize,
             ...
             const BMK_advancedParams_t *adv)
{
    ...
    CHECK_Z(ZSTD_CCtx_setParameter(ctx, ZSTD_c_enableLongDistanceMatching,
                                   adv->ldmFlag));
    CHECK_Z(ZSTD_CCtx_setParameter(ctx, ZSTD_c_ldmMinMatch, adv->ldmMinMatch));
    CHECK_Z(ZSTD_CCtx_setParameter(ctx, ZSTD_c_ldmHashLog, adv->ldmHashLog));
    CHECK_Z(ZSTD_CCtx_setParameter(ctx, ZSTD_c_ldmBucketSizeLog,
                                   adv->ldmBucketSizeLog));
    CHECK_Z(ZSTD_CCtx_setParameter(ctx, ZSTD_c_ldmHashRateLog,
                                   adv->ldmHashRateLog));
    CHECK_Z(
    ...
    CHECK_Z(ZSTD_CCtx_setParameter(ctx, ZSTD_c_targetLength,
...
local_initCCtx(void *payload)
{
    ...
    BMK_initCCtx(ag->cctx, ag->dictBuffer, ag->dictBufferSize, ag->cLevel,
                 ag->comprParams, ag->adv);
    return 0;
}
...
local_defaultDecompress(const void *srcBuffer, size_t srcSize, void *dstBuffer,
                        size_t dstCapacity, void *addArgs)
{
...
int
BMK_isSuccessful_benchOutcome(BMK_benchOutcome_t outcome)
{
    return outcome.tag == 0;
}

BMK_benchResult_t
BMK_extract_benchResult(BMK_benchOutcome_t outcome)
{
...
static BMK_benchOutcome_t
BMK_benchOutcome_setValidResult(BMK_benchResult_t result)
{
    BMK_benchOutcome_t b;
    b.tag = 0;
...
BMK_benchMemAdvancedNoAlloc(
    const void **srcPtrs, size_t *srcSizes, void **cPtrs, size_t *cCapacities,
    size_t *cSizes, void **resPtrs, size_t *resSizes, void **resultBufferPtr,
    void *compressedBuffer, size_t maxCompressedSize,
    BMK_timedFnState_t *timeStateCompress,
    BMK_timedFnState_t *timeStateDecompress,

    const void *srcBuffer, size_t srcSize, const size_t *fileSizes,
    unsigned nbFiles, const int cLevel,
    const ZSTD_compressionParameters *comprParams, const void *dictBuffer,
    size_t dictBufferSize, ZSTD_CCtx *cctx, ZSTD_DCtx *dctx, int displayLevel,
    const char *displayName, const BMK_advancedParams_t *adv)
{
    ...
    if (adv->mode ==
        BMK_decodeOnly) { /* benchmark only decompression : source must be
                             already compressed */
        ...
        for (fileNb = 0; fileNb < nbFiles; fileNb++) {
            ...
            if (fSize64 == 0)
                RETURN_ERROR(32, BMK_benchOutcome_t,
                             "Impossible to determine original size ");
    ...
    {
        U64 const crcOrig =
            (adv->mode == BMK_decodeOnly) ? 0 : XXH64(srcBuffer, srcSize, 0);
#define NB_MARKS 4
        ...
        DISPLAYLEVEL(2, "%2s-%-17.17s :%10u ->\r", marks[markNb], displayName,
                     (unsigned)srcSize);
        ...
        while (!(compressionCompleted && decompressionCompleted)) {
            if (!compressionCompleted) {
...

                {
                    ...
                    ratio = (double)srcSize / cSize;
                    {
                        ...
                        newResult.cSpeed = ((U64)srcSize * TIMELOOP_NANOSEC /
...

                {
                    ...
                    DISPLAYLEVEL(
                        2, "%2s-%-17.17s :%10u ->%10u (%5.*f),%6.*f MB/s\r",
                        marks[markNb], displayName, (unsigned)srcSize,
                        (unsigned)cSize, ratioAccuracy, ratio,
                        benchResult.cSpeed < (10 MB) ? 2 : 1,
            ...
            if (!decompressionCompleted) {
...

                {
                    ...
                    U64 const newDSpeed =
                        (srcSize * TIMELOOP_NANOSEC / dResult.nanoSecPerRun);
...

                {
                    int const ratioAccuracy = (ratio < 10.) ? 3 : 2;
                    DISPLAYLEVEL(2,
                                 "%2s-%-17.17s :%10u ->%10u (%5.*f),%6.*f MB/s "
                                 ",%6.1f MB/s \r",
                                 marks[markNb], displayName, (unsigned)srcSize,
                                 (unsigned)benchResult.cSize, ratioAccuracy,
                                 ratio, benchResult.cSpeed < (10 MB) ? 2 : 1,
                                 (double)benchResult.cSpeed / MB_UNIT,
...
BMK_benchOutcome_t
BMK_benchMemAdvanced(const void *srcBuffer, size_t srcSize, void *dstBuffer,
                     size_t dstCapacity, const size_t *fileSizes,
                     unsigned nbFiles, int cLevel,
                     const ZSTD_compressionParameters *comprParams,
...

{
    ...
    BMK_timedFnState_t *timeStateCompress =
        BMK_createTimedFnState(adv->nbSeconds * 1000, BMK_RUNTEST_DEFAULT_MS);
    BMK_timedFnState_t *timeStateDecompress =
        BMK_createTimedFnState(adv->nbSeconds * 1000, BMK_RUNTEST_DEFAULT_MS);
    ...
    int allocationincomplete =
        !srcPtrs || !srcSizes || !cPtrs || !cSizes || !cCapacities ||
        !resPtrs || !resSizes || !timeStateCompress || !timeStateDecompress ||
        !cctx || !dctx || !compressedBuffer || !resultBuffer;

    if (!allocationincomplete && !dstParamsError) {
        outcome = BMK_benchMemAdvancedNoAlloc(
            srcPtrs, srcSizes, cPtrs, cCapacities, cSizes, resPtrs, resSizes,
            &resultBuffer, compressedBuffer, maxCompressedSize,
            timeStateCompress, timeStateDecompress, srcBuffer, srcSize,
            fileSizes, nbFiles, cLevel, comprParams, dictBuffer, dictBufferSize,
            cctx, dctx, displayLevel, displayName, adv);
    }
    ...
    if (allocationincomplete) {
        RETURN_ERROR(31, BMK_benchOutcome_t,
...
BMK_benchOutcome_t
BMK_benchMem(const void *srcBuffer, size_t srcSize, const size_t *fileSizes,
             unsigned nbFiles, int cLevel,
             const ZSTD_compressionParameters *comprParams,
             const void *dictBuffer, size_t dictBufferSize, int displayLevel,
             const char *displayName)
{
    BMK_advancedParams_t const adv = BMK_initAdvancedParams();
    return BMK_benchMemAdvanced(srcBuffer, srcSize, NULL, 0, fileSizes, nbFiles,
                                cLevel, comprParams, dictBuffer, dictBufferSize,
                                displayLevel, displayName, &adv);
}

static BMK_benchOutcome_t
BMK_benchCLevel(const void *srcBuffer, size_t benchedSize,
                const size_t *fileSizes, unsigned nbFiles, int cLevel,
                const ZSTD_compressionParameters *comprParams,
                const void *dictBuffer, size_t dictBufferSize, int displayLevel,
                const char *displayName, BMK_advancedParams_t const *const adv)
{
    ...
    if (displayLevel == 1 && !adv->additionalParam) /* --quiet mode */
        DISPLAY("bench %s %s: input %u bytes, %u seconds, %u KB blocks\n",
                ...
                (unsigned)benchedSize, adv->nbSeconds,
                (unsigned)(adv->blockSize >> 10));

    return BMK_benchMemAdvanced(srcBuffer, benchedSize, NULL, 0, fileSizes,
                                nbFiles, cLevel, comprParams, dictBuffer,
                                dictBufferSize, displayLevel, displayName, adv);
}

BMK_benchOutcome_t
BMK_syntheticTest(int cLevel, double compressibility,
                  ...
                  int displayLevel, const BMK_advancedParams_t *adv)
{
    ...
    if (!srcBuffer)
        RETURN_ERROR(21, BMK_benchOutcome_t, "not enough memory");

    /* Fill input buffer */
    RDG_genBuffer(srcBuffer, benchedSize, compressibility, 0.0, 0);

    /* Bench */
    snprintf(name, sizeof(name), "Synthetic %2u%%",
             (unsigned)(compressibility * 100));
    res =
        BMK_benchCLevel(srcBuffer, benchedSize, &benchedSize /* ? */, 1 /* ? */,
                        cLevel, compressionParams, NULL, 0, /* dictionary */
...
static int
BMK_loadFiles(void *buffer, size_t bufferSize, size_t *fileSizes,
              ...
              int displayLevel)
{
    ...
    for (n = 0; n < nbFiles; n++) {
        ...
        if (f == NULL)
            EXM_THROW_INT(10, "impossible to open file %s", fileNamesTable[n]);
            ...
            nbFiles = n; /* buffer too small - stop after this file */
        {
            ...
            if (readSize != (size_t)fileSize)
                EXM_THROW_INT(11, "could not read %s", fileNamesTable[n]);
            pos += readSize;
        }
    ...
    if (totalSize == 0)
        EXM_THROW_INT(12, "no data to bench");
    return 0;
}

BMK_benchOutcome_t
BMK_benchFilesAdvanced(const char *const *fileNamesTable, unsigned nbFiles,
                       ...
                       int displayLevel, const BMK_advancedParams_t *adv)
{
    ...
    if (!fileSizes)
        RETURN_ERROR(12, BMK_benchOutcome_t, "not enough memory for fileSizes");
    ...
    if (dictFileName != NULL) {
        ...
        if (dictFileSize > 64 MB) {
            free(fileSizes);
            RETURN_ERROR(10, BMK_benchOutcome_t, "dictionary file %s too large",
                         dictFileName);
        }
        ...
        if (dictBuffer == NULL) {
            free(fileSizes);
            RETURN_ERROR(11, BMK_benchOutcome_t,
        ...
        snprintf(mfName, sizeof(mfName), " %u files", nbFiles);
        {
            ...
            res =
                BMK_benchCLevel(srcBuffer, benchedSize, fileSizes, nbFiles,
                                cLevel, compressionParams, dictBuffer,
                                dictBufferSize, displayLevel, displayName, adv);
        }
    }
...
{
    BMK_advancedParams_t const adv = BMK_initAdvancedParams();
    return BMK_benchFilesAdvanced(fileNamesTable, nbFiles, dictFileName, cLevel,
                                  compressionParams, displayLevel, &adv);
}
