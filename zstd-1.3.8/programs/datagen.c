...
static void
RDG_fillLiteralDistrib(BYTE *ldt, double ld)
{
...
static BYTE
RDG_genChar(U32 *seed, const BYTE *ldt)
{
    U32 const id = RDG_rand(seed) & LTMASK;
    return ldt[id]; /* memory-sanitizer fails here, stating "uninitialized
                       value" when table initialized with P==0.0. Checked :
                       table is fully initialized */
}

static U32
RDG_rand15Bits(U32 *seedPtr)
{
    return RDG_rand(seedPtr) & 0x7FFF;
}

static U32
RDG_randLength(U32 *seedPtr)
{
    if (RDG_rand(seedPtr) & 7)
        return (RDG_rand(seedPtr) & 0xF); /* small length */
    return (RDG_rand(seedPtr) & 0x1FF) + 0xF;
}

static void
RDG_genBlock(void *buffer, size_t buffSize, size_t prefixSize,
             double matchProba, const BYTE *ldt, U32 *seedPtr)
{
    ...
    while (matchProba >= 1.0) {
        ...
        size0 +=
            RDG_rand(seedPtr) & (size0 - 1); /* because size0 is power of 2*/
    ...
    /* init */
    if (pos == 0)
        buffPtr[0] = RDG_genChar(seedPtr, ldt), pos = 1;
    ...
    while (pos < buffSize) {
        ...
        if (RDG_rand15Bits(seedPtr) < matchProba32) {
            ...
            U32 const offset =
                repeatOffset ? prevOffset : (U32)MIN(randOffset, pos);
            size_t match = pos - offset;
            while (pos < d)
                buffPtr[pos++] =
                    buffPtr[match++]; /* correctly manages overlaps */
            prevOffset = offset;
        } else {
            /* Literal (noise) */
            ...
            U32 const d = (U32)MIN(pos + length, buffSize);
            while (pos < d)
                buffPtr[pos++] = RDG_genChar(seedPtr, ldt);
        }
    }
}

void
RDG_genBuffer(void *buffer, size_t size, double matchProba, double litProba,
              unsigned seed)
{
    U32 seed32 = seed;
    BYTE ldt[LTSIZE];
...
void
RDG_genStdout(unsigned long long size, double matchProba, double litProba,
              unsigned seed)
{
    U32 seed32 = seed;
    ...
    while (total < size) {
        ...
        RDG_genBlock(buff, stdDictSize + stdBlockSize, stdDictSize, matchProba,
                     ldt, &seed32);
        total += genBlockSize;
        {
            size_t const unused = fwrite(buff, 1, genBlockSize, stdout);
            (void)unused;
        }
        /* update dict */
