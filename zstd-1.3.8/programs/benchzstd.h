...
typedef struct {
    ...
    unsigned ldmMinMatch; /* below: parameters for long distance matching, see
                             zstd.1.md */
    unsigned ldmHashLog;
 ...
 *  applies ZSTD_compress_generic() and ZSTD_decompress_generic() on data in
 * srcBuffer
 *  with specific compression parameters provided by other arguments using
 * benchFunction
 *  (cLevel, comprParams + adv in advanced Mode) */
/*  srcBuffer - data source, expected to be valid compressed data if in Decode
 * Only Mode
 *  srcSize - size of data in srcBuffer
 *  fileSizes - srcBuffer is considered cut into 1+ segments, to compress
 * separately.
 *              note : sum(fileSizes) must be == srcSize.  (<== ensure it's
 * properly checked)
 ...
 * dstBuffer - destination buffer to write compressed output in, NULL if none
 * provided.
 * dstCapacity - capacity of destination buffer, give 0 if dstBuffer = NULL
 * adv = see advancedParams_t
 */
BMK_benchOutcome_t BMK_benchMemAdvanced(
    const void *srcBuffer, size_t srcSize, void *dstBuffer, size_t dstCapacity,
    const size_t *fileSizes, unsigned nbFiles, int cLevel,
    const ZSTD_compressionParameters *comprParams, const void *dictBuffer,
    size_t dictBufferSize, int displayLevel, const char *displayName,
    const BMK_advancedParams_t *adv);
