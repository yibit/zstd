...
int
BMK_isSuccessful_runOutcome(BMK_runOutcome_t outcome)
{
...
size_t
BMK_extract_errorResult(BMK_runOutcome_t outcome)
{
    assert(outcome.error_tag_never_ever_use_directly != 0);
    return outcome.error_result_never_ever_use_directly;
}

static BMK_runOutcome_t
 ...
 */
BMK_runOutcome_t
BMK_benchFunction(BMK_benchParams_t p, unsigned nbLoops)
{
    size_t dstSize = 0;
    ...
    /* benchmark */
    {
        ...
        for (loopNb = 0; loopNb < nbLoops; loopNb++) {
            for (blockNb = 0; blockNb < p.blockCount; blockNb++) {
                size_t const res =
                    p.benchFn(p.srcBuffers[blockNb], p.srcSizes[blockNb],
                ...
                if (loopNb == 0) {
                    ...
                    if ((p.errorFn != NULL) && (p.errorFn(res))) {
                        RETURN_QUIET_ERROR(
                            ...
                            "Function benchmark failed on block %u (of size "
                            "%u) with error %i",
...
BMK_timedFnState_t *
BMK_createTimedFnState(unsigned total_ms, unsigned run_ms)
{
...
void
BMK_freeTimedFnState(BMK_timedFnState_t *state)
{
    free(state);
}

void
BMK_resetTimedFnState(BMK_timedFnState_t *timedFnState, unsigned total_ms,
                      unsigned run_ms)
{
    if (!total_ms)
        total_ms = 1;
 ...
 * errored. */
int
BMK_isCompleted_TimedFn(const BMK_timedFnState_t *timedFnState)
{
...
BMK_runOutcome_t
BMK_benchTimedFn(BMK_timedFnState_t *cont, BMK_benchParams_t p)
{
    ...
    while (!completed) {
...

        {
            ...
            if (loopDuration_ns > (runBudget_ns / 50)) {
                U64 const fastestRun_ns =
                    MIN(bestRunTime.nanoSecPerRun, newRunTime.nanoSecPerRun);
