...
typedef struct {
    ...
    /*
     * cooldown counter in order to prevent rapid successive decreases in
     * compression level
     * whenever compression level is decreased, cooldown is set to
     * CLEVEL_DECREASE_COOLDOWN
     * whenever adaptCompressionLevel() is called and cooldown != 0, it is
     * decremented
     ...
     * if the value is not 1.0, then this implies that thread X waited on thread
     * Y to finish
     * and thread Y was XWaitYCompletion finished at the time of the wait (i.e.
     * compressWaitWriteCompletion=0.5
     * implies that the compression thread waited on the write thread and it was
     * only 50% finished writing a job)
     */
     ...
     * mini-chunk (i.e. finishes writing out, compressing, etc. this
     * mini-chunk).
     */
...
static void
freeCompressionJobs(adaptCCtx *ctx)
{
    unsigned u;
...
static int
initCCtx(adaptCCtx *ctx, unsigned numJobs)
{
    ...
    if (!ctx->jobs) {
        DISPLAY(
            "Error: could not allocate space for jobs during context "
            "creation\n");
        return 1;
    }
...
static void
signalErrorToThreads(adaptCCtx *ctx)
{
    ctx->threadError = 1;
...
static void
waitUntilAllJobsCompleted(adaptCCtx *ctx)
{
    if (!ctx)
        return;
    pthread_mutex_lock(&ctx->allJobsCompleted_mutex.pMutex);
    while (ctx->allJobsCompleted == 0 && !ctx->threadError) {
        pthread_cond_wait(&ctx->allJobsCompleted_cond.pCond,
                          &ctx->allJobsCompleted_mutex.pMutex);
    }
    pthread_mutex_unlock(&ctx->allJobsCompleted_mutex.pMutex);
}
...
static unsigned
convertCompletionToChange(double completion)
{
...
/*
 * Compression level is changed depending on which part of the compression
 * process is lagging
 * Currently, three theads exist for job creation, compression, and file writing
 * respectively.
 * adaptCompressionLevel() increments or decrements compression level based on
 * which of the threads is lagging
 * job creation or file writing lag => increased compression level
 * compression thread lag           => decreased compression level
 * detecting which thread is lagging is done by keeping track of how many calls
 * each thread makes to pthread_cond_wait
 */
static void
adaptCompressionLevel(adaptCCtx *ctx)
{
    double createWaitCompressionCompletion;
    double compressWaitCreateCompletion;
    double compressWaitWriteCompletion;
    ...
    assert(g_minCLevel <= prevCompressionLevel &&
           g_maxCLevel >= prevCompressionLevel);

    /* adaptation logic */
    if (ctx->cooldown)
        ctx->cooldown--;

    if ((1 - createWaitCompressionCompletion > threshold ||
         1 - writeWaitCompressionCompletion > threshold) &&
        ctx->cooldown == 0) {
        /* create or write waiting on compression */
        /* use whichever one waited less because it was slower */
        double const completion = MAX(createWaitCompressionCompletion,
                                      writeWaitCompressionCompletion);
        unsigned const change = convertCompletionToChange(completion);
        unsigned const boundChange =
            MIN(change, prevCompressionLevel - g_minCLevel);
        if (ctx->convergenceCounter >= CONVERGENCE_LOWER_BOUND &&
            boundChange != 0) {
            ...
            DEBUG(2,
                  "create or write threads waiting on compression, tried to "
                  "decrease compression level by %u\n\n",
                  boundChange);
        }
    } else if (1 - compressWaitWriteCompletion > threshold ||
               1 - compressWaitCreateCompletion > threshold) {
        /* compress waiting on write */
        double const completion =
            MIN(compressWaitWriteCompletion, compressWaitCreateCompletion);
        unsigned const change = convertCompletionToChange(completion);
        unsigned const boundChange =
            MIN(change, g_maxCLevel - prevCompressionLevel);
        if (ctx->convergenceCounter >= CONVERGENCE_LOWER_BOUND &&
            boundChange != 0) {
            /* reset convergence counter, might have been a spike */
            ctx->convergenceCounter = 0;
            ...
            DEBUG(2,
                  "compress waiting on write or create, tried to increase "
                  "compression level by %u\n\n",
                  boundChange);
        }
    }
...
static size_t
getUseableDictSize(unsigned compressionLevel)
{
    ZSTD_parameters const params = ZSTD_getParams(compressionLevel, 0, 0);
    unsigned const overlapLog =
        compressionLevel >= (unsigned)ZSTD_maxCLevel() ? 0 : 3;
    size_t const overlapSize = 1 << (params.cParams.windowLog - overlapLog);
    return overlapSize;
}

static void *
compressionThread(void *arg)
{
    ...
    for (;;) {
...

        {
            ...
            pthread_mutex_lock(&ctx->jobReady_mutex.pMutex);
            if (currJob + 1 > ctx->jobReadyID)
                willWaitForCreate = 1;
            pthread_mutex_unlock(&ctx->jobReady_mutex.pMutex);

            pthread_mutex_lock(&ctx->jobWrite_mutex.pMutex);
            if (currJob - ctx->jobWriteID >= ctx->numJobs)
                willWaitForWrite = 1;
            pthread_mutex_unlock(&ctx->jobWrite_mutex.pMutex);
        ...
        while (currJob + 1 > ctx->jobReadyID && !ctx->threadError) {
            pthread_cond_wait(&ctx->jobReady_cond.pCond,
                              &ctx->jobReady_mutex.pMutex);
        }
        pthread_mutex_unlock(&ctx->jobReady_mutex.pMutex);
        ...
        while (currJob - ctx->jobWriteID >= ctx->numJobs && !ctx->threadError) {
            pthread_cond_wait(&ctx->jobWrite_cond.pCond,
                              &ctx->jobWrite_mutex.pMutex);
        }
        pthread_mutex_unlock(&ctx->jobWrite_mutex.pMutex);
        ...
        /* compress the data */
        {
            ...
            /* begin compression */
            {
                size_t const useDictSize =
                    MIN(getUseableDictSize(cLevel), job->dictSize);
                ...
                params.cParams.windowLog = 23;
                {
                    size_t const initError = ZSTD_compressBegin_advanced(
                        ctx->cctx, job->src.start + job->dictSize - useDictSize,
                        useDictSize, params, 0);
                    size_t const windowSizeError = ZSTD_CCtx_setParameter(
                        ctx->cctx, ZSTD_c_forceMaxWindow, 1);
                    if (ZSTD_isError(initError) ||
                        ZSTD_isError(windowSizeError)) {
                        DISPLAY(
                            "Error: something went wrong while starting "
                            "compression\n");
            ...
            do {
                size_t const actualBlockSize =
                    MIN(remaining, compressionBlockSize);

                /* continue compression */
                if (currJob != 0 ||
                    blockNum != 0) { /* not first block of first job
                                        flush/overwrite the frame header */
                    size_t const hSize = ZSTD_compressContinue(
                        ctx->cctx, job->dst.start + dstPos,
                        job->dst.capacity - dstPos,
                        job->src.start + job->dictSize + srcPos, 0);
                    if (ZSTD_isError(hSize)) {
                        DISPLAY(
                            "Error: something went wrong while continuing "
                            "compression\n");
                ...
                {
                    size_t const ret =
                        (job->lastJobPlusOne == currJob + 1 &&
                         remaining == actualBlockSize)
                            ? ZSTD_compressEnd(
                                  ctx->cctx, job->dst.start + dstPos,
                                  job->dst.capacity - dstPos,
                                  job->src.start + job->dictSize + srcPos,
                                  actualBlockSize)
                    ...
                    if (ZSTD_isError(ret)) {
                        DISPLAY(
                            "Error: something went wrong during compression: "
                            "%s\n",
                    ...
                    /* update completion */
                    pthread_mutex_lock(
                        &ctx->compressionCompletion_mutex.pMutex);
                    ctx->compressionCompletion =
                        1 - (double)remaining / job->src.size;
...
static void
displayProgress(unsigned cLevel, unsigned last)
{
        ...
        return;
    {
        double const timeElapsed =
            (double)(UTIL_getSpanTimeMicro(g_startTime, currTime) / 1000.0);
        double const sizeMB = (double)g_streamedSize / (1 << 20);
        double const avgCompRate = sizeMB * 1000 / timeElapsed;
        fprintf(stderr,
                "\r| Comp. Level: %2u | Time Elapsed: %7.2f s | Data Size: "
                "%7.1f MB | Avg Comp. Rate: %6.2f MB/s |",
                cLevel, timeElapsed / 1000.0, sizeMB, avgCompRate);
        if (last) {
            fprintf(stderr, "\n");
        } else {
            fflush(stderr);
...
outputThread(void *arg)
{
    ...
    for (;;) {
        ...
        pthread_mutex_lock(&ctx->jobCompressed_mutex.pMutex);
        if (currJob + 1 > ctx->jobCompressedID)
            willWaitForCompress = 1;
        pthread_mutex_unlock(&ctx->jobCompressed_mutex.pMutex);
        ...
        if (willWaitForCompress) {
            ...
            DEBUG(2,
                  "writer thread waiting for nextJob: %u, "
                  "writeWaitCompressionCompletion %f\n",
        ...
        while (currJob + 1 > ctx->jobCompressedID && !ctx->threadError) {
            pthread_cond_wait(&ctx->jobCompressed_cond.pCond,
...

        {
            ...
            }
            {
                ...
                for (;;) {
                    ...
                    size_t const ret =
                        fwrite(job->dst.start + pos, 1, writeSize, dstFile);
                    if (ret != writeSize)
                        break;
                    pos += ret;
                    remaining -= ret;
                    ...
                    pthread_mutex_lock(&ctx->writeCompletion_mutex.pMutex);
                    ctx->writeCompletion =
                        1 - (double)remaining / compressedSize;
                    pthread_mutex_unlock(&ctx->writeCompletion_mutex.pMutex);
                ...
                if (pos != compressedSize) {
                    DISPLAY(
                        "Error: an error occurred during file write "
                        "operation\n");
                    signalErrorToThreads(ctx);
                    return arg;
                }
...
static int
createCompressionJob(adaptCCtx *ctx, size_t srcSize, int last)
{
...
static int
performCompression(adaptCCtx *ctx, FILE *const srcFile, outputThreadArg *otArg)
{
    ...
    }
    {
        ...
        for (;;) {
            ...
            pthread_mutex_lock(&ctx->jobCompressed_mutex.pMutex);
            if (nextJob - ctx->jobCompressedID >= ctx->numJobs)
                willWaitForCompress = 1;
            pthread_mutex_unlock(&ctx->jobCompressed_mutex.pMutex);
            ...
            if (willWaitForCompress) {
                ...
                ctx->createWaitCompressionCompletion =
                    ctx->compressionCompletion;
                DEBUG(2,
                      "create thread waiting for nextJob: %u, "
                      "createWaitCompressionCompletion %f\n",
            ...
            pthread_mutex_lock(&ctx->jobCompressed_mutex.pMutex);
            while (nextJob - ctx->jobCompressedID >= ctx->numJobs &&
                   !ctx->threadError) {
                pthread_cond_wait(&ctx->jobCompressed_cond.pCond,
                                  &ctx->jobCompressed_mutex.pMutex);
            }
            ...
            while (remaining != 0 && !feof(srcFile)) {
                size_t const ret =
                    fread(ctx->input.buffer.start + ctx->input.filled + pos, 1,
                          readBlockSize, srcFile);
                if (ret != readBlockSize && !feof(srcFile)) {
                    /* error could not read correct number of bytes */
                    DISPLAY(
                        "Error: problem occurred during read from src file\n");
                    signalErrorToThreads(ctx);
                    return 1;
                }
                pos += ret;
                ...
                ctx->createCompletion =
                    1 - (double)remaining / ((size_t)FILE_CHUNK_SIZE);
                pthread_mutex_unlock(&ctx->createCompletion_mutex.pMutex);
            }
                               ...
                               const char *const dstFilenameOrNull)
{
    ...
    const char *const outFilenameIntermediate =
        (stdinUsed && !dstFilenameOrNull) ? stdoutmark : dstFilenameOrNull;
    ...
    if (!outFilenameIntermediate) {
        if (snprintf(fileAndSuffix, MAX_PATH, "%s.zst", srcFilename) + 1 >
            MAX_PATH) {
...

    {
        ...
        if (!fcr.otArg) {
            DISPLAY(
                "Error: could not allocate space for output thread argument\n");
            return fcr;
        }
...
static int
freeFileCompressionResources(fcResources *fcr)
{
    int ret = 0;
    ...
    if (fcr->otArg) {
        ret |=
            (fcr->otArg->dstFile != stdout) ? fclose(fcr->otArg->dstFile) : 0;
        free(fcr->otArg);
        /* no need to freeCCtx() on otArg->ctx because it should be the same
         * context */
    }
    return ret;
}
...
static int
compressFilename(const char *const srcFilename,
                 const char *const dstFilenameOrNull)
{
    int ret = 0;
    fcResources fcr =
        createFileCompressionResources(srcFilename, dstFilenameOrNull);
    g_streamedSize = 0;
...
static int
compressFilenames(const char **filenameTable, unsigned numFiles,
...
help(const char *progPath)
{
    ...
    PRINT(
        "  -i#    : provide initial compression level -- default %d, must be "
        "in the range [L, U] where L and U are bound values (see below for "
        "defaults)\n",
    ...
    PRINT(
        "  -q     : quiet mode -- do not show progress bar or other "
        "information\n");
    PRINT(
        "  -l#    : provide lower bound for compression level -- default 1\n");
    PRINT(
        "  -u#    : provide upper bound for compression level -- default %u\n",
        ZSTD_maxCLevel());
}
...
main(int argCount, const char *argv[])
{
    ...
    const char **filenameTable =
        (const char **)malloc(argCount * sizeof(const char *));
    ...
    /* check initial, max, and min compression levels */
    {
        ...
        unsigned const initialNotInRange = g_minCLevel > g_compressionLevel ||
                                           g_maxCLevel < g_compressionLevel;
        if (minMaxInconsistent ||
            (initialNotInRange && providedInitialCLevel)) {
            DISPLAY(
                "Error: provided compression level parameters are invalid\n");
            ret = 1;
            goto _main_exit;
    ...
    if (filenameIdx > 1 &&
        (outFilename != NULL && strcmp(outFilename, stdoutmark))) {
        DISPLAY(
            "Error: multiple input files provided, cannot use specified output "
            "file\n");
        ret = 1;
        goto _main_exit;
    }
