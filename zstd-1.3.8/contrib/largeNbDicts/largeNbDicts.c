...
static void
freeBuffer(buffer_t buff)
{
    free(buff.ptr);
}

static void
fillBuffer_fromHandle(buffer_t *buff, FILE *f)
{
...
/* @return : kBuffNull if any error */
static buffer_t
createBuffer_fromFile(const char *fileName)
{
...
/* @return : kBuffNull if any error */
static buffer_t
createDictionaryBuffer(const char *dictionaryName, const void *srcBuffer,
                       const size_t *srcBlockSizes, size_t nbBlocks,
                       size_t requestedDictSize)
{
    if (dictionaryName) {
        DISPLAYLEVEL(3, "loading dictionary %s \n", dictionaryName);
        return createBuffer_fromFile(
            dictionaryName); /* note : result might be kBuffNull */
        ...
        size_t const dictSize =
            ZDICT_trainFromBuffer(dictBuffer, requestedDictSize, srcBuffer,
...
static int
loadFiles(void *buffer, size_t bufferSize, size_t *fileSizes,
...
static void
freeSliceCollection(slice_collection_t collection)
{
...
/* splitSlices() :
 * nbSlices : if == 0, nbSlices is automatically determined from srcSlices and
 * blockSize.
...
{
    if (blockSize == 0)
        blockSize = (size_t)(-1); /* means "do not cut" */
    ...
    for (size_t sliceNb = 0; sliceNb < nbSlices;) {
        ...
        while (pos < srcSlices.capacities[ssnb] && sliceNb < nbSlices) {
            size_t const size =
                MIN(blockSize, srcSlices.capacities[ssnb] - pos);
...
static size_t
sliceCollection_totalCapacity(slice_collection_t sc)
{
    size_t totalSize = 0;
...
static void
freeBufferCollection(buffer_collection_t bc)
{
                                 ...
                                 unsigned nbFiles)
{
    ...
    /* Load input buffer */
    int const errorCode =
        loadFiles(srcBuffer, loadedSize, fileSizes, fileNamesTable, nbFiles);
    assert(errorCode == 0);
...
static void
freeDDictCollection(ddict_collection_t ddictc)
{
...
static ddict_collection_t
createDDictCollection(const void *dictBuffer, size_t dictSize, size_t nbDDict)
{
    ZSTD_DDict **const ddicts = malloc(nbDDict * sizeof(ZSTD_DDict *));
    assert(ddicts != NULL);
    if (ddicts == NULL)
        return kNullDDictCollection;
...
/* mess with adresses, so that linear scanning dictionaries != linear address
 * scanning */
void
shuffleDictionaries(ddict_collection_t dicts)
{
 ...
 */
static size_t
compressBlocks(size_t *cSizes, /* optional (can be NULL). If present, must
                                  contain at least nbBlocks fields */
               slice_collection_t dstBlockBuffers,
               slice_collection_t srcBlockBuffers, ZSTD_CDict *cdict,
               int cLevel)
{
    ...
    for (size_t blockNb = 0; blockNb < nbBlocks; blockNb++) {
        ...
        if (cdict == NULL) {
            cBlockSize =
                ZSTD_compressCCtx(cctx, dstBlockBuffers.slicePtrs[blockNb],
                                  dstBlockBuffers.capacities[blockNb],
                                  srcBlockBuffers.slicePtrs[blockNb],
                                  srcBlockBuffers.capacities[blockNb], cLevel);
        } else {
            cBlockSize = ZSTD_compress_usingCDict(
                cctx, dstBlockBuffers.slicePtrs[blockNb],
                dstBlockBuffers.capacities[blockNb],
                srcBlockBuffers.slicePtrs[blockNb],
                srcBlockBuffers.capacities[blockNb], cdict);
        }
        CONTROL(!ZSTD_isError(cBlockSize));
        if (cSizes)
            cSizes[blockNb] = cBlockSize;
...
decompressInstructions
createDecompressInstructions(ddict_collection_t dictionaries)
{
...
void
freeDecompressInstructions(decompressInstructions di)
{
...
/* benched function */
size_t
decompress(const void *src, size_t srcSize, void *dst, size_t dstCapacity,
           void *payload)
{
    decompressInstructions *const di = (decompressInstructions *)payload;

    size_t const result =
        ZSTD_decompress_usingDDict(di->dctx, dst, dstCapacity, src, srcSize,
                                   di->dictionaries.ddicts[di->dictNb]);

    di->dictNb = di->dictNb + 1;
    if (di->dictNb >= di->nbDicts)
        di->dictNb = 0;
...
static int
benchMem(slice_collection_t dstBlocks, slice_collection_t srcBlocks,
         ddict_collection_t dictionaries, int nbRounds)
{
    ...
    for (;;) {
        ...
        if (dSpeed_MBps > bestSpeed)
            bestSpeed = dSpeed_MBps;
        DISPLAY("Decompression Speed : %.1f MB/s \r", bestSpeed);
        fflush(stdout);
        if (BMK_isCompleted_TimedFn(benchState))
            break;
    }
    DISPLAY("\n");
...
int
bench(const char **fileNameTable, unsigned nbFiles, const char *dictionary,
      ...
      int nbRounds)
{
    ...
    buffer_collection_t const srcs =
        createBufferCollection_fromFiles(fileNameTable, nbFiles);
    ...
    slice_collection_t const srcSlices =
        splitSlices(srcs.slices, blockSize, nbBlocks);
    ...
    buffer_t const dictBuffer = createDictionaryBuffer(
        dictionary, srcs.buffer.ptr, srcs.slices.capacities,
        srcs.slices.nbSlices, DICTSIZE);
    ...
    size_t const cTotalSizeNoDict =
        compressBlocks(NULL, dstSlices, srcSlices, NULL, clevel);
    CONTROL(cTotalSizeNoDict != 0);
    DISPLAYLEVEL(3,
                 "compressing at level %u without dictionary : Ratio=%.2f  (%u "
                 "bytes) \n",
    ...
    size_t const cTotalSize =
        compressBlocks(cSizes, dstSlices, srcSlices, cdict, clevel);
    CONTROL(cTotalSize != 0);
    DISPLAYLEVEL(
        3, "compressed using a %u bytes dictionary : Ratio=%.2f  (%u bytes) \n",
        (unsigned)dictBuffer.size, (double)totalSrcSlicesSize / cTotalSize,
        (unsigned)cTotalSize);

    /* now dstSlices contain the real compressed size of each block, instead of
     * the maximum capacity */
    shrinkSizes(dstSlices, cSizes);

    size_t const dictMem =
        ZSTD_estimateDDictSize(dictBuffer.size, ZSTD_dlm_byCopy);
    ...
    ddict_collection_t const dictionaries =
        createDDictCollection(dictBuffer.ptr, dictBuffer.size, nbDicts);
    CONTROL(dictionaries.ddicts != NULL);

    shuffleDictionaries(dictionaries);

    buffer_collection_t resultCollection =
        createBufferCollection_fromSliceCollectionSizes(srcSlices);
    CONTROL(resultCollection.buffer.ptr != NULL);

    result =
        benchMem(resultCollection.slices, dstSlices, dictionaries, nbRounds);
...
usage(const char *exeName)
{
    ...
    DISPLAY("Options : \n");
    DISPLAY(
        "-r          : recursively load all files in subdirectories (default: "
        "off) \n");
    DISPLAY(
    ...
    DISPLAY(
        "--nbDicts=# : create # dictionaries for bench (default: one per "
        "block) \n");
...
main(int argc, const char **argv)
{
    ...
    unsigned nbDicts =
        0; /* determine nbDicts automatically: 1 dictionary per block */
    unsigned nbBlocks =
        0; /* determine nbBlocks automatically, from source and blockSize */
    ...
    for (int argNb = 1; argNb < argc; argNb++) {
        ...
        if (!strcmp(argument, "-D")) {
            argNb++;
            assert(argNb < argc);
            dictionary = argv[argNb];
            continue;
        }
        if (longCommandWArg(&argument, "-i")) {
            nbRounds = readU32FromChar(&argument);
            continue;
        }
        if (longCommandWArg(&argument, "--dictionary=")) {
            dictionary = argument;
            continue;
        }
        if (longCommandWArg(&argument, "-B")) {
            blockSize = readU32FromChar(&argument);
            continue;
        }
        if (longCommandWArg(&argument, "--blockSize=")) {
            blockSize = readU32FromChar(&argument);
            continue;
        }
        if (longCommandWArg(&argument, "--nbDicts=")) {
            nbDicts = readU32FromChar(&argument);
            continue;
        }
        if (longCommandWArg(&argument, "--nbBlocks=")) {
            nbBlocks = readU32FromChar(&argument);
            continue;
        }
        if (longCommandWArg(&argument, "--clevel=")) {
...
#endif
        filenameTable = UTIL_createFileList(nameTable, nameIdx,
                                            &buffer_containing_filenames,
                                            &nbFiles, 1 /* follow_links */);
    }

    int result = bench(filenameTable, nbFiles, dictionary, blockSize, cLevel,
                       nbDicts, nbBlocks, nbRounds);
