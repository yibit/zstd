...
zstd_get_btrfs_parameters(size_t src_len)
{
    ZSTD_parameters params =
        ZSTD_getParams(ZSTD_BTRFS_DEFAULT_LEVEL, src_len, 0);
...
static void
zstd_free_workspace(struct list_head *ws)
{
...
static int
zstd_compress_pages(struct list_head *ws, struct address_space *mapping,
    ...
    *total_in = 0;
    ...
    while (1) {
        ...
        if (tot_in + in_buf.pos > 8192 &&
            tot_in + in_buf.pos < tot_out + out_buf.pos) {
            ret = -E2BIG;
            goto out;
        }
...
static int
zstd_decompress_bio(struct list_head *ws, struct compressed_bio *cb)
{
    struct workspace *workspace = list_entry(ws, struct workspace, list);
    ...
    stream =
        ZSTD_initDStream(ZSTD_BTRFS_MAX_INPUT, workspace->mem, workspace->size);
    if (!stream) {
    ...
    while (1) {
        ...
        ret = btrfs_decompress_buf2page(out_buf.dst, buf_start, total_out,
                                        disk_start, orig_bio);
        if (ret == 0)
            break;
...
static int
zstd_decompress(struct list_head *ws, unsigned char *data_in,
