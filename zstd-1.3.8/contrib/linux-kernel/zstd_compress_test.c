...
static int
zstd_compress_test_open(struct inode *i, struct file *f)
{
    if (device_is_open)
        return -EBUSY;
...
static int
zstd_compress_test_release(struct inode *i, struct file *f)
{
 ...
 */
static ssize_t
zstd_compress_test_write(struct file *file, const char __user *buf, size_t size,
                         loff_t *pos)
{
