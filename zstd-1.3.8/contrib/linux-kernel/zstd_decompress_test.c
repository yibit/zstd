...
static ZSTD_inBuffer input = {
    .src = buffer_in, .size = sizeof(buffer_in), .pos = sizeof(buffer_in),
};

static ZSTD_outBuffer output = {
    .dst = buffer_out, .size = sizeof(buffer_out), .pos = sizeof(buffer_out),
};
...
static int
zstd_decompress_test_open(struct inode *i, struct file *f)
{
    if (device_is_open)
        return -EBUSY;
...
static int
zstd_decompress_test_release(struct inode *i, struct file *f)
{
 ...
 */
static ssize_t
zstd_decompress_test_write(struct file *file, const char __user *buf,
...
static int __init
zstd_decompress_test_init(void)
{
    ...
    printk(KERN_INFO DEVICE_NAME
           ": decompression requires %zu bytes of memory\n",
           workspace_size);
    printk(KERN_INFO DEVICE_NAME
...
static void __exit
zstd_decompress_test_exit(void)
{
