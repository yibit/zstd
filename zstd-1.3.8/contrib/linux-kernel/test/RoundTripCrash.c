              ...
              size_t srcBuffSize)
{
    ...
    size_t const cSize =
        ZSTD_compressCCtx(cctx, compressedBuff, compressedBuffCapacity, srcBuff,
                          srcBuffSize, params);
    if (ZSTD_isError(cSize)) {
        fprintf(stderr, "Compression error : %u \n", ZSTD_getErrorCode(cSize));
        return cSize;
    }
    return ZSTD_decompressDCtx(dctx, resultBuff, resultBuffCapacity,
                               compressedBuff, cSize);
}
...
static void
roundTripCheck(const void *srcBuff, size_t srcBuffSize)
{
    ...
    if (!cctx) {
        ZSTD_compressionParameters const params =
            ZSTD_getCParams(kMaxClevel, 0, 0);
    ...
    {
        size_t const result = roundTripTest(rBuff, buffSize, cBuff, buffSize,
                                            srcBuff, srcBuffSize);
