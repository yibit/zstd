...
***********************************************************/
#if !defined(__64BIT__) || defined(__MINGW32__) /* No point defining Large \
                                                   file for 64 bit but     \
                                                   MinGW-w64 requires it */
#if !defined(_FILE_OFFSET_BITS)
#define _FILE_OFFSET_BITS \
    64 /* turn off_t into a 64-bit type for ftello, fseeko */
#endif
#if !defined( \
    _LARGEFILE_SOURCE) /* obsolete macro, replaced with _FILE_OFFSET_BITS */
#define _LARGEFILE_SOURCE \
    1 /* Large File Support extension (LFS) - fseeko, ftello */
#endif
...
static int
ZSTD_seekable_read_FILE(void *opaque, void *buffer, size_t n)
{
...
static int
ZSTD_seekable_seek_FILE(void *opaque, long long offset, int origin)
{
...
static int
ZSTD_seekable_read_buff(void *opaque, void *buffer, size_t n)
{
    buffWrapper_t *buff = (buffWrapper_t *)opaque;
    if (buff->size + n > buff->pos)
        return -1;
...
static int
ZSTD_seekable_seek_buff(void *opaque, long long offset, int origin)
{
...
struct ZSTD_seekable_s {
    ...
    BYTE outBuff[SEEKABLE_BUFF_SIZE]; /* so we can efficiently decompress the
                                         ...
                                         desired section */
    ZSTD_inBuffer
        in; /* maintain continuity across ZSTD_seekable_decompress operations */
...
size_t
ZSTD_seekable_free(ZSTD_seekable *zs)
{
    if (zs == NULL)
...
unsigned
ZSTD_seekable_offsetToFrameIndex(ZSTD_seekable *const zs,
...
unsigned
ZSTD_seekable_getNumFrames(ZSTD_seekable *const zs)
{
    assert(zs->seekTable.tableLen <= UINT_MAX);
...
unsigned long long
ZSTD_seekable_getFrameCompressedOffset(ZSTD_seekable *const zs,
                                       unsigned frameIndex)
{
...
unsigned long long
ZSTD_seekable_getFrameDecompressedOffset(ZSTD_seekable *const zs,
                                         unsigned frameIndex)
{
    if (frameIndex >= zs->seekTable.tableLen)
        return ZSTD_SEEKABLE_FRAMEINDEX_TOOLARGE;
...
size_t
ZSTD_seekable_getFrameCompressedSize(ZSTD_seekable *const zs,
                                     unsigned frameIndex)
{
    if (frameIndex >= zs->seekTable.tableLen)
...
size_t
ZSTD_seekable_getFrameDecompressedSize(ZSTD_seekable *const zs,
                                       unsigned frameIndex)
{
    if (frameIndex > zs->seekTable.tableLen)
        return ERROR(frameIndex_tooLarge);
...
static size_t
ZSTD_seekable_loadSeekTable(ZSTD_seekable *zs)
{
    int checksumFlag;
...

    {
        ...
        U32 const frameSize =
            tableSize + ZSTD_seekTableFooterSize + ZSTD_SKIPPABLEHEADERSIZE;

        U32 remaining =
            frameSize -
            ZSTD_seekTableFooterSize; /* don't need to re-read footer */
        {
        ...
        if (MEM_readLE32(zs->inBuff + 4) + ZSTD_SKIPPABLEHEADERSIZE !=
            frameSize) {
            return ERROR(prefix_unknown);
        }
           ...
           * computations on the last element without special case */
            ...
            for (; idx < numFrames; idx++) {
                if (pos + sizePerEntry > SEEKABLE_BUFF_SIZE) {
                    ...
                    U32 const toRead =
                        MIN(remaining, SEEKABLE_BUFF_SIZE - offset);
                    memmove(zs->inBuff, zs->inBuff + pos,
                            offset); /* move any data we haven't read yet */
...
size_t
ZSTD_seekable_initBuff(ZSTD_seekable *zs, const void *src, size_t srcSize)
{
...
size_t
ZSTD_seekable_initFile(ZSTD_seekable *zs, FILE *src)
{
...
size_t
ZSTD_seekable_initAdvanced(ZSTD_seekable *zs, ZSTD_seekable_customFile src)
{
    zs->src = src;

    {
        const size_t seekTableInit = ZSTD_seekable_loadSeekTable(zs);
        if (ZSTD_isError(seekTableInit))
...
size_t
ZSTD_seekable_decompress(ZSTD_seekable *zs, void *dst, size_t len,
    ...
    do {
        ...
        while (zs->decompressedOffset < offset + len) {
            ...
            if (zs->decompressedOffset < offset) {
                ...
                outTmp = (ZSTD_outBuffer){
                    zs->outBuff,
                    MIN(SEEKABLE_BUFF_SIZE, offset - zs->decompressedOffset),
                    0};
            } else {
                outTmp =
                    (ZSTD_outBuffer){dst, len, zs->decompressedOffset - offset};
            }
            ...
            if (toRead == 0) {
                ...
                if (zs->decompressedOffset < offset + len) {
                    ...
                    targetFrame = ZSTD_seekable_offsetToFrameIndex(
                        zs, zs->decompressedOffset);
                }
                break;
            }
...
size_t
ZSTD_seekable_decompressFrame(ZSTD_seekable *zs, void *dst, size_t dstSize,
                              unsigned frameIndex)
{
    if (frameIndex >= zs->seekTable.tableLen) {
        return ERROR(frameIndex_tooLarge);
