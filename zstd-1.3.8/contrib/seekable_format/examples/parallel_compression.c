...
static void
compressFrame(void *opaque)
{
    struct job *job = opaque;

    job->checksum = XXH64(job->src, job->srcSize, 0);

    size_t ret = ZSTD_compress(job->dst, job->dstSize, job->src, job->srcSize,
...
compressFile_orDie(const char *fname, const char *outName, int cLevel,
                   unsigned frameSize, int nbThreads)
{
    ...
    for (i = 0; i < numFrames; i++) {
        ...
        size_t ret = ZSTD_seekable_logFrame(fl, jobs[i].dstSize,
                                            jobs[i].srcSize, jobs[i].checksum);
        if (ZSTD_isError(ret)) {
