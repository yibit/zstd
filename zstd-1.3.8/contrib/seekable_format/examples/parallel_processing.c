...
static void *
realloc_orDie(void *ptr, size_t size)
{
...
static FILE *
fopen_orDie(const char *filename, const char *instruction)
{
...
static size_t
fread_orDie(void *buffer, size_t sizeToRead, FILE *file)
{
...
static size_t
fwrite_orDie(const void *buffer, size_t sizeToWrite, FILE *file)
{
...
static void
fseek_orDie(FILE *file, long int offset, int origin)
{
...
sumFrame(void *opaque)
{
    ...
    size_t const frameSize =
        ZSTD_seekable_getFrameDecompressedSize(seekable, job->frameNb);
    unsigned char *data = malloc_orDie(frameSize);

    size_t result =
        ZSTD_seekable_decompressFrame(seekable, data, frameSize, job->frameNb);
    if (ZSTD_isError(result)) {
        fprintf(stderr, "ZSTD_seekable_decompressFrame() error : %s \n",
...
static void
sumFile_orDie(const char *fname, int nbThreads)
{
    ...
    struct sum_job *jobs =
        (struct sum_job *)malloc(numFrames * sizeof(struct sum_job));
