...
size_t
ZSTD_seekable_frameLog_allocVec(ZSTD_frameLog *fl)
{
    /* allocate some initial space */
    size_t const FRAMELOG_STARTING_CAPACITY = 16;
    fl->entries = (framelogEntry_t *)malloc(sizeof(framelogEntry_t) *
...
size_t
ZSTD_seekable_frameLog_freeVec(ZSTD_frameLog *fl)
{
    if (fl != NULL)
        free(fl->entries);
    return 0;
...
size_t
ZSTD_seekable_freeFrameLog(ZSTD_frameLog *fl)
{
...
ZSTD_seekable_createCStream()
{
    ...
    if (ZSTD_isError(ZSTD_seekable_frameLog_allocVec(&zcs->framelog)))
        goto failed2;
...
size_t
ZSTD_seekable_freeCStream(ZSTD_seekable_CStream *zcs)
{
    if (zcs == NULL)
...
size_t
ZSTD_seekable_initCStream(ZSTD_seekable_CStream *zcs, int compressionLevel,
                          int checksumFlag, unsigned maxFrameSize)
{
    zcs->framelog.size = 0;
    zcs->frameCSize = 0;
    zcs->frameDSize = 0;
    ...
    zcs->maxFrameSize =
        maxFrameSize ? maxFrameSize : ZSTD_SEEKABLE_MAX_FRAME_DECOMPRESSED_SIZE;
...
size_t
ZSTD_seekable_logFrame(ZSTD_frameLog *fl, unsigned compressedSize,
                       unsigned decompressedSize, unsigned checksum)
{
    ...
    if (fl->size == fl->capacity) {
        ...
        framelogEntry_t *const newEntries =
            realloc(fl->entries, sizeof(framelogEntry_t) * newCapacity);
    ...
    fl->entries[fl->size] =
        (framelogEntry_t){compressedSize, decompressedSize, checksum};
    fl->size++;
...
size_t
ZSTD_seekable_endFrame(ZSTD_seekable_CStream *zcs, ZSTD_outBuffer *output)
{
    ...
    ret = ZSTD_seekable_logFrame(
        ...
        zcs->framelog.checksumFlag ? XXH64_digest(&zcs->xxhState) & 0xFFFFFFFFU
                                   : 0);
    if (ret)
        return ret;
...
size_t
ZSTD_seekable_compressStream(ZSTD_seekable_CStream *zcs, ZSTD_outBuffer *output,
                             ZSTD_inBuffer *input)
{
    ...
    /* if we haven't finished flushing the last frame, don't start writing a new
     * one */
    if (inLen > 0) {
...
ZSTD_seekable_seekTableSize(const ZSTD_frameLog *fl)
{
...
static inline size_t
ZSTD_stwrite32(ZSTD_frameLog *fl, ZSTD_outBuffer *output, U32 const value,
               U32 const offset)
{
    if (fl->seekTablePos < offset + 4) {
        BYTE tmp[4]; /* so that we can work with buffers too small to write a
                        whole word to */
        ...
        if (lenWrite < 4)
            return ZSTD_seekable_seekTableSize(fl) - fl->seekTablePos;
    }
    return 0;
}

size_t
ZSTD_seekable_writeSeekTable(ZSTD_frameLog *fl, ZSTD_outBuffer *output)
{
    ...
    CHECK_Z(ZSTD_stwrite32(fl, output,
                           (U32)seekTableLen - ZSTD_SKIPPABLEHEADERSIZE, 4));

    while (fl->seekTableIndex < fl->size) {
        unsigned long long const start =
            ZSTD_SKIPPABLEHEADERSIZE + sizePerFrame * fl->seekTableIndex;
        assert(start + 8 <= UINT_MAX);
        CHECK_Z(ZSTD_stwrite32(
            fl, output, fl->entries[fl->seekTableIndex].cSize, (U32)start + 0));

        CHECK_Z(ZSTD_stwrite32(
            fl, output, fl->entries[fl->seekTableIndex].dSize, (U32)start + 4));

        if (fl->checksumFlag) {
            CHECK_Z(ZSTD_stwrite32(fl, output,
                                   fl->entries[fl->seekTableIndex].checksum,
                                   (U32)start + 8));
        }
    ...
    if (output->size - output->pos < 1)
        return seekTableLen - fl->seekTablePos;
    ...
    if (fl->seekTablePos != seekTableLen)
        return ERROR(GENERIC);
    return 0;
}

size_t
ZSTD_seekable_endStream(ZSTD_seekable_CStream *zcs, ZSTD_outBuffer *output)
{
    if (!zcs->writingSeekTable && zcs->frameDSize) {
        const size_t endFrame = ZSTD_seekable_endFrame(zcs, output);
        if (ZSTD_isError(endFrame))
            return endFrame;
        /* return an accurate size hint */
        if (endFrame)
            return endFrame + ZSTD_seekable_seekTableSize(&zcs->framelog);
    }
