 ...
 */
static unsigned
loadFiles(void *buffer, size_t *bufferSizePtr, size_t *sampleSizes,
...
static void
shuffle(const char **fileNamesTable, unsigned nbFiles)
{
    U32 seed = 0xFD2FB528;
    unsigned i;
...
**********************************************************/
size_t
findMaxMem(unsigned long long requiredMem)
{
...
void
saveDict(const char *dictFileName, const void *buff, size_t buffSize)
{
 ...
 */
static fileStats
getFileStats(const char **fileNamesTable, unsigned nbFiles, size_t chunkSize,
...
sampleInfo *
getSampleInfo(const char **fileNamesTable, unsigned nbFiles, size_t chunkSize,
              unsigned maxDictSize, const unsigned displayLevel)
{
    fileStats const fs =
        getFileStats(fileNamesTable, nbFiles, chunkSize, displayLevel);
...
void
freeSampleInfo(sampleInfo *info)
{
    if (!info)
        return;
    if (info->samplesSizes)
        free((void *)(info->samplesSizes));
    if (info->srcBuffer)
        free((void *)(info->srcBuffer));
    free(info);
}
