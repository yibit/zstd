...
***************************************/
/** createDictFromFiles() :
 *  Based on type of param given, train dictionary using the corresponding
 * algorithm
 *  @return dictInfo containing dictionary buffer and dictionary size
 */
dictInfo *
createDictFromFiles(sampleInfo *info, unsigned maxDictSize,
                    ZDICT_random_params_t *randomParams,
                    ZDICT_cover_params_t *coverParams,
                    ZDICT_legacy_params_t *legacyParams,
                    ZDICT_fastCover_params_t *fastParams)
{
    unsigned const displayLevel =
        randomParams
            ? randomParams->zParams.notificationLevel
            : coverParams
                  ? coverParams->zParams.notificationLevel
                  : legacyParams
                        ? legacyParams->zParams.notificationLevel
    ...
    if (!dictBuffer)
        EXM_THROW(
            12, "not enough memory for trainFromFiles"); /* should not happen */

    {
        size_t dictSize;
        if (randomParams) {
            dictSize = ZDICT_trainFromBuffer_random(
                dictBuffer, maxDictSize, info->srcBuffer, info->samplesSizes,
                info->nbSamples, *randomParams);
        } else if (coverParams) {
            ...
            if (!coverParams->d || !coverParams->k) {
                dictSize = ZDICT_optimizeTrainFromBuffer_cover(
                    dictBuffer, maxDictSize, info->srcBuffer,
...
/** compressWithDict() :
 *  Compress samples from sample buffer given dicionary stored on dictionary
 * buffer and compression level
 *  @return compression ratio
 */
double
compressWithDict(sampleInfo *srcInfo, dictInfo *dInfo, int compressionLevel,
                 int displayLevel)
{
    /* Local variables */
    ...
    if (hasDict) {
        cdict = ZSTD_createCDict(dInfo->dictBuffer, dInfo->dictSize,
                                 compressionLevel);
        if (!cdict) {
            cRatio = -1;
            goto _cleanup;
    ...
    for (i = 0; i < srcInfo->nbSamples; i++) {
        ...
        if (hasDict) {
            compressedSize = ZSTD_compress_usingCDict(
                ...
                srcInfo->samplesSizes[i], cdict);
        } else {
            compressedSize =
                ZSTD_compressCCtx(cctx, dst, dstCapacity, samples + offsets[i],
                                  srcInfo->samplesSizes[i], compressionLevel);
        }
 ...
 */
void
freeDictInfo(dictInfo *info)
{
    if (!info)
        return;
    if (info->dictBuffer)
        free((void *)(info->dictBuffer));
    free(info);
}
 ...
 *  Measure how long a dictionary builder takes and compression ratio with the
 * dictionary built
 *  @return 0 if benchmark successfully, 1 otherwise
 */
int
benchmarkDictBuilder(sampleInfo *srcInfo, unsigned maxDictSize,
                     ...
                     ZDICT_fastCover_params_t *fastParam)
{
    ...
    const unsigned displayLevel =
        randomParam
            ? randomParam->zParams.notificationLevel
            : coverParam
                  ? coverParam->zParams.notificationLevel
                  : legacyParam
                        ? legacyParam->zParams.notificationLevel
    ...
    const char *name =
        randomParam
            ...
            : coverParam
                  ? "COVER"
                  : legacyParam ? "LEGACY"
                                : fastParam ? "FAST" : "NODICT"; /* no dict */
    const unsigned cLevel =
        randomParam
            ? randomParam->zParams.compressionLevel
            : coverParam ? coverParam->zParams.compressionLevel
                         : legacyParam
                               ? legacyParam->zParams.compressionLevel
    ...
    dictInfo *dInfo = createDictFromFiles(srcInfo, maxDictSize, randomParam,
                                          coverParam, legacyParam, fastParam);
    ...
    /* Calculate compression ratio */
    const double cRatio =
        compressWithDict(srcInfo, dInfo, cLevel, displayLevel);
    if (cRatio < 0) {
...
main(int argCount, const char *argv[])
{
    ...
    extendedFileList = UTIL_createFileList(
        filenameTable, filenameIdx, &fileNamesBuf, &fileNamesNb, followLinks);
    if (extendedFileList) {
        unsigned u;
    ...
    /* get sampleInfo */
    size_t blockSize = 0;
    sampleInfo *srcInfo = getSampleInfo(filenameTable, filenameIdx, blockSize,
                                        maxDictSize, displayLevel);
    ...
    /* with no dict */
    {
        const int noDictResult =
            benchmarkDictBuilder(srcInfo, maxDictSize, NULL, NULL, NULL, NULL);
        if (noDictResult) {
            result = 1;
            goto _cleanup;
        }
    ...
    /* for random */
    {
        ...
        randomParam.k = k;
        const int randomResult = benchmarkDictBuilder(
            srcInfo, maxDictSize, &randomParam, NULL, NULL, NULL);
        DISPLAYLEVEL(2, "k=%u\n", randomParam.k);
        if (randomResult) {
            result = 1;
            goto _cleanup;
    ...
    /* for legacy */
    {
        ...
        const int legacyResult = benchmarkDictBuilder(
            srcInfo, maxDictSize, NULL, NULL, &legacyParam, NULL);
    ...
    /* for cover */
    {
        ...
        const int coverOptResult = benchmarkDictBuilder(
            srcInfo, maxDictSize, NULL, &coverParam, NULL, NULL);
        DISPLAYLEVEL(2, "k=%u\nd=%u\nsteps=%u\nsplit=%u\n", coverParam.k,
                     coverParam.d, coverParam.steps,
                     (unsigned)(coverParam.splitPoint * 100));
        if (coverOptResult) {
            result = 1;
            goto _cleanup;
        }
        ...
        const int coverResult = benchmarkDictBuilder(srcInfo, maxDictSize, NULL,
                                                     &coverParam, NULL, NULL);
        DISPLAYLEVEL(2, "k=%u\nd=%u\nsteps=%u\nsplit=%u\n", coverParam.k,
                     coverParam.d, coverParam.steps,
                     (unsigned)(coverParam.splitPoint * 100));
        if (coverResult) {
    ...
    for (f = 15; f < 25; f++) {
        ...
        for (accel = 1; accel < 11; accel++) {
            ...
            const int fastOptResult = benchmarkDictBuilder(
                srcInfo, maxDictSize, NULL, NULL, NULL, &fastParam);
            DISPLAYLEVEL(2, "k=%u\nd=%u\nf=%u\nsteps=%u\nsplit=%u\naccel=%u\n",
                         fastParam.k, fastParam.d, fastParam.f, fastParam.steps,
                         (unsigned)(fastParam.splitPoint * 100),
                         fastParam.accel);
            if (fastOptResult) {
                result = 1;
                goto _cleanup;
            ...
            for (i = 0; i < 5; i++) {
                const int fastResult = benchmarkDictBuilder(
                    srcInfo, maxDictSize, NULL, NULL, NULL, &fastParam);
                DISPLAYLEVEL(
                    2, "k=%u\nd=%u\nf=%u\nsteps=%u\nsplit=%u\naccel=%u\n",
                    fastParam.k, fastParam.d, fastParam.f, fastParam.steps,
                    (unsigned)(fastParam.splitPoint * 100), fastParam.accel);
                if (fastResult) {
                    result = 1;
                    goto _cleanup;
                }
