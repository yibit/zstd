...
#define FASTCOVER_MAX_SAMPLES_SIZE \
    (sizeof(size_t) == 8 ? ((U32)-1) : ((U32)1 GB))
...
static size_t
ZSTD_hash6(U64 u, U32 h)
{
    return (size_t)(((u << (64 - 48)) * prime6bytes) >> (64 - h));
}
static size_t
ZSTD_hash6Ptr(const void *p, U32 h)
{
...
static size_t
ZSTD_hash8(U64 u, U32 h)
{
    return (size_t)(((u)*prime8bytes) >> (64 - h));
}
static size_t
ZSTD_hash8Ptr(const void *p, U32 h)
{
 ...
 */
static size_t
FASTCOVER_hashPtrToIndex(const void *p, U32 h, unsigned d)
{
    if (d == 6) {
 ...
 */
static size_t
FASTCOVER_sum(const size_t *samplesSizes, unsigned nbSamples)
{
    size_t sum = 0;
    unsigned i;
 ...
 * Let S_i be hash value of the dmer at position i of segment S which has length
 * k.
 *
 ...
 */
static FASTCOVER_segment_t
FASTCOVER_selectSegment(const FASTCOVER_ctx_t *ctx, U32 *freqs, U32 begin,
                        U32 end, ZDICT_fastCover_params_t parameters)
{
    /* Constants */
    ...
    activeSegment.score = 0;
    {
        ...
        while (activeSegment.end < end) {
            ...
            const size_t index = FASTCOVER_hashPtrToIndex(
                ctx->samples + activeSegment.end, parameters.f, ctx->d);
            /* Add frequency of this index to score if this is the first
             * occurence of index in active segment */
            ...
            if (activeSegment.end - activeSegment.begin == dmersInK + 1) {
                /* Get hash value of the dmer to be eliminated from active
                 * segment */
                const size_t delIndex = FASTCOVER_hashPtrToIndex(
                    ctx->samples + activeSegment.begin, parameters.f, ctx->d);
                ctx->segmentFreqs[delIndex] -= 1;
                /* Subtract frequency of this index from score if this is the
                 * last occurrence of this index in active segment */
    ...
    }
    {
        ...
        for (pos = bestSegment.begin; pos != bestSegment.end; ++pos) {
            const size_t index = FASTCOVER_hashPtrToIndex(ctx->samples + pos,
                                                          parameters.f, ctx->d);
    ...
    }
    {
        ...
        U32 pos;
        for (pos = bestSegment.begin; pos != bestSegment.end; ++pos) {
            const size_t i = FASTCOVER_hashPtrToIndex(ctx->samples + pos,
                                                      parameters.f, ctx->d);
            freqs[i] = 0;
        }
    }
 ...
 */
static int
FASTCOVER_checkParameters(ZDICT_fastCover_params_t parameters,
                          size_t maxDictSize)
{
 ...
 */
static void
FASTCOVER_computeFrequency(U32 *freqs, unsigned f, FASTCOVER_ctx_t *ctx)
{
    ...
    for (unsigned i = 0; i < ctx->nbTrainSamples; i++) {
        ...
        while (start + ctx->d <= currSampleEnd) {
            const size_t dmerIndex =
                FASTCOVER_hashPtrToIndex(ctx->samples + start, f, ctx->d);
            freqs[dmerIndex]++;
            start++;
        }
    }
 ...
 */
static int
FASTCOVER_ctx_init(FASTCOVER_ctx_t *ctx, const void *samplesBuffer,
                   const size_t *samplesSizes, unsigned nbSamples, unsigned d,
                   double splitPoint, unsigned f)
{
    const BYTE *const samples = (const BYTE *)samplesBuffer;
    const size_t totalSamplesSize = FASTCOVER_sum(samplesSizes, nbSamples);
    /* Split samples into testing and training sets */
    const unsigned nbTrainSamples =
        splitPoint < 1.0 ? (unsigned)((double)nbSamples * splitPoint)
                         : nbSamples;
    const unsigned nbTestSamples =
        splitPoint < 1.0 ? nbSamples - nbTrainSamples : nbSamples;
    const size_t trainingSamplesSize =
        splitPoint < 1.0 ? FASTCOVER_sum(samplesSizes, nbTrainSamples)
                         : totalSamplesSize;
    const size_t testSamplesSize =
        splitPoint < 1.0
            ? FASTCOVER_sum(samplesSizes + nbTrainSamples, nbTestSamples)
            : totalSamplesSize;
    /* Checks */
 ...
 */
static size_t
FASTCOVER_buildDictionary(const FASTCOVER_ctx_t *ctx, U32 *freqs,
 ...
 */
static void
FASTCOVER_best_init(FASTCOVER_best_t *best)
{
    if (best == NULL)
 ...
 */
static void
FASTCOVER_best_wait(FASTCOVER_best_t *best)
{
    if (!best) {
        return;
    }
 ...
 */
static void
FASTCOVER_best_destroy(FASTCOVER_best_t *best)
{
    if (!best) {
        return;
    }
 ...
 */
static void
FASTCOVER_best_start(FASTCOVER_best_t *best)
{
    if (!best) {
        return;
    }
 ...
 */
static void
FASTCOVER_best_finish(FASTCOVER_best_t *best, size_t compressedSize,
 ...
 */
static void
FASTCOVER_tryParameters(void *opaque)
{
    /* Save parameters as local variables */
    FASTCOVER_tryParameters_data_t *const data =
        (FASTCOVER_tryParameters_data_t *)opaque;
    ...
    {
        const size_t tail = FASTCOVER_buildDictionary(
            ctx, freqs, dict, dictBufferCapacity, parameters);
                                ...
                                ZDICT_fastCover_params_t parameters)
{
    ...
    {
        const size_t tail = FASTCOVER_buildDictionary(
            &ctx, ctx.freqs, dictBuffer, dictBufferCapacity, parameters);
...
ZDICTLIB_API size_t
ZDICT_optimizeTrainFromBuffer_fastCover(void *dictBuffer,
                                        size_t dictBufferCapacity,
                                        const void *samplesBuffer,
                                        const size_t *samplesSizes,
                                        unsigned nbSamples,
                                        ZDICT_fastCover_params_t *parameters)
{
    ...
    for (d = kMinD; d <= kMaxD; d += 2) {
        ...
        if (!FASTCOVER_ctx_init(&ctx, samplesBuffer, samplesSizes, nbSamples, d,
                                splitPoint, f)) {
        ...
        for (k = kMinK; k <= kMaxK; k += kStepSize) {
            ...
            FASTCOVER_tryParameters_data_t *data =
                (FASTCOVER_tryParameters_data_t *)malloc(
            ...
            if (!FASTCOVER_checkParameters(data->parameters,
                                           dictBufferCapacity)) {
                DISPLAYLEVEL(1, "fastCover parameters incorrect\n");
                free(data);
                continue;
